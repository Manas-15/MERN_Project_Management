...
import PurchaseDocumentsInlineForm from './PurchaseDocumentsInlineForm';
import PurchaseDocumentsGridForm from './PurchaseDocumentsGridForm';
import PurchaseDocumentsGlobalForm from './PurchaseDocumentsGlobalForm';
import { 
...
const PurchaseDocumentsListing : React.FC<{
  ...
  useEffect(() => {
   ...
   const {
    data: fetchedTrnInvPurchaseDocumentData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchTrnInvPurchaseDocumentList
  } = useGetAllTrnInvPurchaseDocumentsTrnInvPurchaseDocumentGetQuery(
    {
      ...tenantIDHeader(),
     //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedTrnInvPurchaseDocumentData) {
      setIsDataReady(true);
      setRowCount(fetchedTrnInvPurchaseDocumentData.meta?.total_items);
    }
  }, [fetchedTrnInvPurchaseDocumentData, isLoadingData, isLoadingError]);
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/dashboard/procurement/purchase-documents/${row.original.id}`)}
                      >
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/dashboard/procurement/purchase-documents/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnInvPurchaseDocumentList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnInvPurchaseDocument({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnInvPurchaseDocumentUpdate: inpdata
      }).unwrap();

      toast.success('Purchase Documents updated successfully');
      refetchTrnInvPurchaseDocumentList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <PurchaseDocumentsGridForm
              setOpen={setIsCreatePopupOpen}
              refetchList={refetchTrnInvPurchaseDocumentList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this PurchaseDocuments ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedTrnInvPurchaseDocumentData ?? []}
            rowCount={rowCount}
...
export default PurchaseDocumentsListing;