...
import PurchaseOrderItemsInlineForm from './PurchaseOrderItemsInlineForm';
import PurchaseOrderItemsGridForm from './PurchaseOrderItemsGridForm';
import PurchaseOrderItemsGlobalForm from './PurchaseOrderItemsGlobalForm';
import { 
...
const PurchaseOrderItemsListing : React.FC<{
  ...
  useEffect(() => {
   ...
   const {
    data: fetchedTrnPurchaseOrderItemsData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchTrnPurchaseOrderItemsList
  } = useGetAllTrnPurchaseOrderItemssTrnPurchaseOrderItemsGetQuery(
    {
      ...tenantIDHeader(),
     //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedTrnPurchaseOrderItemsData) {
      setIsDataReady(true);
      setRowCount(fetchedTrnPurchaseOrderItemsData.meta?.total_items);
    }
  }, [fetchedTrnPurchaseOrderItemsData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
      () => [
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/dashboard/procurement/purchase-order-items/${row.original.id}`)}
                      >
          ...
          {
              accessorKey: 'requsition_qty',
              header: `Requsition Qty`,
              size: 150,
              enableSorting: true,
              enableColumnFilter: false,
              enableEditing: true,
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/dashboard/procurement/purchase-order-items/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnPurchaseOrderItemsList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnPurchaseOrderItems({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnPurchaseOrderItemsUpdate: inpdata
      }).unwrap();

      toast.success('Purchase Order Items updated successfully');
      refetchTrnPurchaseOrderItemsList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <PurchaseOrderItemsGridForm
              setOpen={setIsCreatePopupOpen}
              refetchList={refetchTrnPurchaseOrderItemsList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this PurchaseOrderItems ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedTrnPurchaseOrderItemsData ?? []}
            rowCount={rowCount}
