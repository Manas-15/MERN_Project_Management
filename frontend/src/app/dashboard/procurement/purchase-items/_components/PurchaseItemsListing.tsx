...
import PurchaseItemsInlineForm from './PurchaseItemsInlineForm';
import PurchaseItemsGridForm from './PurchaseItemsGridForm';
import PurchaseItemsGlobalForm from './PurchaseItemsGlobalForm';
import { 
...
const PurchaseItemsListing : React.FC<{
  ...
  useEffect(() => {
   ...
   const {
    data: fetchedTrnInvPurchaseItemsData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchTrnInvPurchaseItemsList
  } = useGetAllTrnInvPurchaseItemssTrnInvPurchaseItemsGetQuery(
    {
      ...tenantIDHeader(),
     //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedTrnInvPurchaseItemsData) {
      setIsDataReady(true);
      setRowCount(fetchedTrnInvPurchaseItemsData.meta?.total_items);
    }
  }, [fetchedTrnInvPurchaseItemsData, isLoadingData, isLoadingError]);
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/dashboard/procurement/purchase-items/${row.original.id}`)}
                      >
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/dashboard/procurement/purchase-items/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnInvPurchaseItemsList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnInvPurchaseItems({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnInvPurchaseItemsUpdate: inpdata
      }).unwrap();

      toast.success('Purchase Items updated successfully');
      refetchTrnInvPurchaseItemsList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <PurchaseItemsGridForm
              setOpen={setIsCreatePopupOpen}
              refetchList={refetchTrnInvPurchaseItemsList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this PurchaseItems ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedTrnInvPurchaseItemsData ?? []}
            rowCount={rowCount}
...
export default PurchaseItemsListing;