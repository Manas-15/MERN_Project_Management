...
import PurchaseOrderTermsInlineForm from './PurchaseOrderTermsInlineForm';
import PurchaseOrderTermsGridForm from './PurchaseOrderTermsGridForm';
import PurchaseOrderTermsGlobalForm from './PurchaseOrderTermsGlobalForm';
import { 
...
const PurchaseOrderTermsListing : React.FC<{
  ...
  useEffect(() => {
   ...
   const {
    data: fetchedTrnPurchaseOrderTermsData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchTrnPurchaseOrderTermsList
  } = useGetAllTrnPurchaseOrderTermssTrnPurchaseOrderTermsGetQuery(
    {
      ...tenantIDHeader(),
     //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedTrnPurchaseOrderTermsData) {
      setIsDataReady(true);
      setRowCount(fetchedTrnPurchaseOrderTermsData.meta?.total_items);
    }
  }, [fetchedTrnPurchaseOrderTermsData, isLoadingData, isLoadingError]);
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/dashboard/procurement/purchase-order-terms/${row.original.id}`)}
                      >
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/dashboard/procurement/purchase-order-terms/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnPurchaseOrderTermsList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnPurchaseOrderTerms({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnPurchaseOrderTermsUpdate: inpdata
      }).unwrap();

      toast.success('Purchase Order Terms updated successfully');
      refetchTrnPurchaseOrderTermsList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <PurchaseOrderTermsGridForm
              setOpen={setIsCreatePopupOpen}
              refetchList={refetchTrnPurchaseOrderTermsList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this PurchaseOrderTerms ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedTrnPurchaseOrderTermsData ?? []}
            rowCount={rowCount}
...
export default PurchaseOrderTermsListing;