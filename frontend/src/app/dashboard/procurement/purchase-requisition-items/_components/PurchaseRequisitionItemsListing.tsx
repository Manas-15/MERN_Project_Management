...
import PurchaseRequisitionItemsInlineForm from './PurchaseRequisitionItemsInlineForm';
import PurchaseRequisitionItemsGridForm from './PurchaseRequisitionItemsGridForm';
import PurchaseRequisitionItemsGlobalForm from './PurchaseRequisitionItemsGlobalForm';
import { 
...
const PurchaseRequisitionItemsListing : React.FC<{
  ...
  useEffect(() => {
   ...
   const {
    data: fetchedTrnPurchaseReqItemsData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchTrnPurchaseReqItemsList
  } = useGetAllTrnPurchaseReqItemssTrnPurchaseReqItemsGetQuery(
    {
      ...tenantIDHeader(),
     //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedTrnPurchaseReqItemsData) {
      setIsDataReady(true);
      setRowCount(fetchedTrnPurchaseReqItemsData.meta?.total_items);
    }
  }, [fetchedTrnPurchaseReqItemsData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
      () => [
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/dashboard/procurement/purchase-requisition-items/${row.original.id}`)}
                      >
          ...
          {
              accessorKey: 'requisition_quantity',
              header: `Requisition Quantity`,
              size: 150,
              enableSorting: true,
              enableColumnFilter: false,
              enableEditing: true,
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/dashboard/procurement/purchase-requisition-items/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnPurchaseReqItemsList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnPurchaseReqItems({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnPurchaseReqItemsUpdate: inpdata
      }).unwrap();

      toast.success('Purchase Requisition Items updated successfully');
      refetchTrnPurchaseReqItemsList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <PurchaseRequisitionItemsGridForm
              setOpen={setIsCreatePopupOpen}
              refetchList={refetchTrnPurchaseReqItemsList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this PurchaseRequisitionItems ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedTrnPurchaseReqItemsData ?? []}
            rowCount={rowCount}
...
export default PurchaseRequisitionItemsListing;