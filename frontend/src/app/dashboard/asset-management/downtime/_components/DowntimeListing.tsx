...
import DowntimeInlineForm from './DowntimeInlineForm';
import DowntimeGridForm from './DowntimeGridForm';
import DowntimeGlobalForm from './DowntimeGlobalForm';
import {
...
const DowntimeListing: React.FC<{
  ...
  const {
    data: fetchedTrnDowntimeData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchTrnDowntimeList
  } = useGetAllTrnDowntimesTrnDowntimeGetQuery(
    {
      ...tenantIDHeader()
      //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedTrnDowntimeData) {
      setIsDataReady(true);
      setRowCount(fetchedTrnDowntimeData.meta?.total_items);
    }
  }, [fetchedTrnDowntimeData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
    () => [
          ...
          return (
            <div className='flex items-center'>
              <div
                ...
                onClick={() =>
                  router.push(
                    `/dashboard/asset-management/downtime/${row.original.id}`
                  )
                }
              >
      ...
      {
        accessorKey: 'shift_meter_reading',
        header: `Shift Meter Reading`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
      ...
      {
        accessorKey: 'shift_drum_reading',
        header: `Shift Drum Reading`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
      ...
      {
        accessorKey: 'downtime_type_id',
        header: `Downtime Type Id`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
      ...
      {
        accessorKey: 'downtime_start_time',
        header: `Downtime Start Time`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
      ...
      {
        accessorKey: 'downtime_end_time',
        header: `Downtime End Time`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
      ...
      {
        accessorKey: 'downtime_section',
        header: `Downtime Section`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
        router.push(`/dashboard/asset-management/downtime/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnDowntimeList(); // Call refetch here
  ...
  const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnDowntime({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnDowntimeUpdate: inpdata
      }).unwrap();

      toast.success('Downtime updated successfully');
      refetchTrnDowntimeList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
          <DowntimeGridForm
            ...
            refetchList={refetchTrnDowntimeList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this Downtime ?`}
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
          <MuiTanstackTable
            tableTitle=''
            columns={columns}
            importedData={fetchedTrnDowntimeData ?? []}
            rowCount={rowCount}
...
export default DowntimeListing;
