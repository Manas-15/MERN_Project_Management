...
import AssetMovementsInlineForm from './AssetMovementsInlineForm';
import AssetMovementsGridForm from './AssetMovementsGridForm';
import AssetMovementsGlobalForm from './AssetMovementsGlobalForm';
import {
...
const AssetMovementsListing: React.FC<{
  ...
  const {
    data: fetchedTrnAssetMovmentData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchTrnAssetMovmentList
  } = useGetAllTrnAssetMovmentsTrnAssetMovmentGetQuery(
    {
      ...tenantIDHeader()
      //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedTrnAssetMovmentData) {
      setIsDataReady(true);
      setRowCount(fetchedTrnAssetMovmentData.meta?.total_items);
    }
  }, [fetchedTrnAssetMovmentData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
    () => [
          ...
          return (
            <div className='flex items-center'>
              <div
                ...
                onClick={() =>
                  router.push(
                    `/dashboard/asset-management/asset-movements/${row.original.id}`
                  )
                }
              >
      ...
      {
        accessorKey: 'site_change_date',
        header: `Site Change Date`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
      ...
      {
        accessorKey: 'from_asset_name',
        header: `From Asset Name`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
      ...
      {
        accessorKey: 'to_asset_name',
        header: `To Asset Name`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnAssetMovmentList(); // Call refetch here
  ...
  const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnAssetMovment({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnAssetMovmentUpdate: inpdata
      }).unwrap();

      toast.success('Asset Movements updated successfully');
      refetchTrnAssetMovmentList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
          <AssetMovementsGridForm
            setOpen={setIsCreatePopupOpen}
            refetchList={refetchTrnAssetMovmentList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this AssetMovements ?`}
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
          <MuiTanstackTable
            tableTitle=''
            columns={columns}
            importedData={fetchedTrnAssetMovmentData ?? []}
            rowCount={rowCount}
...
export default AssetMovementsListing;
