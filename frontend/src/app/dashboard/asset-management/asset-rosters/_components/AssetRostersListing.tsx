...
import AssetRostersInlineForm from './AssetRostersInlineForm';
import AssetRostersGridForm from './AssetRostersGridForm';
import AssetRostersGlobalForm from './AssetRostersGlobalForm';
import {
...
const AssetRostersListing: React.FC<{
  ...
  const {
    data: fetchedTrnAssetRosterData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchTrnAssetRosterList
  } = useGetAllTrnAssetRostersTrnAssetRosterGetQuery(
    {
      ...tenantIDHeader()
      //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedTrnAssetRosterData) {
      setIsDataReady(true);
      setRowCount(fetchedTrnAssetRosterData.meta?.total_items);
    }
  }, [fetchedTrnAssetRosterData, isLoadingData, isLoadingError]);
          ...
          return (
            <div className='flex items-center'>
              <div
                ...
                onClick={() =>
                  router.push(
                    `/dashboard/asset-management/asset-rosters/${row.original.id}`
                  )
                }
              >
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnAssetRosterList(); // Call refetch here
  ...
  const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnAssetRoster({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnAssetRosterUpdate: inpdata
      }).unwrap();

      toast.success('Asset Rosters updated successfully');
      refetchTrnAssetRosterList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
          <AssetRostersGridForm
            setOpen={setIsCreatePopupOpen}
            refetchList={refetchTrnAssetRosterList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this AssetRosters ?`}
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
          <MuiTanstackTable
            tableTitle=''
            columns={columns}
            importedData={fetchedTrnAssetRosterData ?? []}
            rowCount={rowCount}
...
export default AssetRostersListing;
