...
import EngagementsInlineForm from './EngagementsInlineForm';
import EngagementsGridForm from './EngagementsGridForm';
import EngagementsGlobalForm from './EngagementsGlobalForm';
import {
...
const EngagementsListing: React.FC<{
  ...
  const {
    data: fetchedTrnEngagementData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchTrnEngagementList
  } = useGetAllTrnEngagementsTrnEngagementGetQuery(
    {
      ...tenantIDHeader()
      //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedTrnEngagementData) {
      setIsDataReady(true);
      setRowCount(fetchedTrnEngagementData.meta?.total_items);
    }
  }, [fetchedTrnEngagementData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
    () => [
          ...
          return (
            <div className='flex items-center'>
              <div
                ...
                onClick={() =>
                  router.push(
                    `/dashboard/asset-management/engagements/${row.original.id}`
                  )
                }
              >
      ...
      {
        accessorKey: 'engagement_duration',
        header: `Engagement Duration`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
      ...
      {
        accessorKey: 'engagement_fullshift',
        header: `Engagement Fullshift`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
      ...
      {
        accessorKey: 'engagement_start_time',
        header: `Engagement Start Time`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
      ...
      {
        accessorKey: 'engagement_end_time',
        header: `Engagement End Time`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnEngagementList(); // Call refetch here
  ...
  const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnEngagement({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnEngagementUpdate: inpdata
      }).unwrap();

      toast.success('Engagements updated successfully');
      refetchTrnEngagementList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
          <EngagementsGridForm
            ...
            refetchList={refetchTrnEngagementList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this Engagements ?`}
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
          <MuiTanstackTable
            tableTitle=''
            columns={columns}
            importedData={fetchedTrnEngagementData ?? []}
            rowCount={rowCount}
...
export default EngagementsListing;
