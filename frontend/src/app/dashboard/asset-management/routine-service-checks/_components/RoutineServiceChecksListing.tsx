...
import RoutineServiceChecksInlineForm from './RoutineServiceChecksInlineForm';
import RoutineServiceChecksGridForm from './RoutineServiceChecksGridForm';
import RoutineServiceChecksGlobalForm from './RoutineServiceChecksGlobalForm';
import {
...
const RoutineServiceChecksListing: React.FC<{
  ...
  const {
    data: fetchedTrnRoutineServiceCheckData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchTrnRoutineServiceCheckList
  } = useGetAllTrnRoutineServiceChecksTrnRoutineServiceCheckGetQuery(
    {
      ...tenantIDHeader()
      //orderBy: orderBy,
  ...
  useEffect(() => {
    if (
      !isLoadingData &&
      !isLoadingError &&
      fetchedTrnRoutineServiceCheckData
    ) {
      setIsDataReady(true);
      setRowCount(fetchedTrnRoutineServiceCheckData.meta?.total_items);
    }
  }, [fetchedTrnRoutineServiceCheckData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
    () => [
          ...
          return (
            <div className='flex items-center'>
              <div
                ...
                onClick={() =>
                  router.push(
                    `/dashboard/asset-management/routine-service-checks/${row.original.id}`
                  )
                }
              >
      ...
      {
        accessorKey: 'check_sheet_id',
        header: `Check Sheet Id`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
      ...
      {
        accessorKey: 'previous_counter_log_hours',
        header: `Previous Counter Log Hours`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
      ...
      {
        accessorKey: 'current_counter_log_hours',
        header: `Current Counter Log Hours`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
      ...
      {
        accessorKey: 'previous_drum_hours',
        header: `Previous Drum Hours`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
      ...
      {
        accessorKey: 'current_drum_hours',
        header: `Current Drum Hours`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnRoutineServiceCheckList(); // Call refetch here
  ...
  const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnRoutineServiceCheck({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnRoutineServiceCheckUpdate: inpdata
      }).unwrap();

      toast.success('Routine Service Checks updated successfully');
      refetchTrnRoutineServiceCheckList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
          <RoutineServiceChecksGridForm
            setOpen={setIsCreatePopupOpen}
            refetchList={refetchTrnRoutineServiceCheckList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this RoutineServiceChecks ?`}
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
          <MuiTanstackTable
            tableTitle=''
            columns={columns}
            importedData={fetchedTrnRoutineServiceCheckData ?? []}
            rowCount={rowCount}
...
export default RoutineServiceChecksListing;
