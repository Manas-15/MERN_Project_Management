...
import AssetSiteLinkingInlineForm from './AssetSiteLinkingInlineForm';
import AssetSiteLinkingGridForm from './AssetSiteLinkingGridForm';
import AssetSiteLinkingGlobalForm from './AssetSiteLinkingGlobalForm';
import {
...
const AssetSiteLinkingListing: React.FC<{
  ...
  const {
    data: fetchedTrnAssetSiteLinkingData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchTrnAssetSiteLinkingList
  } = useGetAllTrnAssetSiteLinkingsTrnAssetSiteLinkingGetQuery(
    {
      ...tenantIDHeader()
      //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedTrnAssetSiteLinkingData) {
      setIsDataReady(true);
      setRowCount(fetchedTrnAssetSiteLinkingData.meta?.total_items);
    }
  }, [fetchedTrnAssetSiteLinkingData, isLoadingData, isLoadingError]);
          ...
          return (
            <div className='flex items-center'>
              <div
                ...
                onClick={() =>
                  router.push(
                    `/dashboard/asset-management/asset-site-linking/${row.original.id}`
                  )
                }
              >
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnAssetSiteLinkingList(); // Call refetch here
  ...
  const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnAssetSiteLinking({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnAssetSiteLinkingUpdate: inpdata
      }).unwrap();

      toast.success('Asset Site Linking updated successfully');
      refetchTrnAssetSiteLinkingList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
          <AssetSiteLinkingGridForm
            setOpen={setIsCreatePopupOpen}
            refetchList={refetchTrnAssetSiteLinkingList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this AssetSiteLinking ?`}
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
          <MuiTanstackTable
            tableTitle=''
            columns={columns}
            importedData={fetchedTrnAssetSiteLinkingData ?? []}
            rowCount={rowCount}
...
export default AssetSiteLinkingListing;
