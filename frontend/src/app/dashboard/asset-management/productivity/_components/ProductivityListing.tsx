...
import ProductivityInlineForm from './ProductivityInlineForm';
import ProductivityGridForm from './ProductivityGridForm';
import ProductivityGlobalForm from './ProductivityGlobalForm';
import {
...
const ProductivityListing: React.FC<{
  ...
  const {
    data: fetchedTrnProductivityData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchTrnProductivityList
  } = useGetAllTrnProductivitysTrnProductivityGetQuery(
    {
      ...tenantIDHeader()
      //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedTrnProductivityData) {
      setIsDataReady(true);
      setRowCount(fetchedTrnProductivityData.meta?.total_items);
    }
  }, [fetchedTrnProductivityData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
    () => [
          ...
          return (
            <div className='flex items-center'>
              <div
                ...
                onClick={() =>
                  router.push(
                    `/dashboard/asset-management/productivity/${row.original.id}`
                  )
                }
              >
      ...
      {
        accessorKey: 'activity_name_id',
        header: `Activity Name Id`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
      ...
      {
        accessorKey: 'net_productivity',
        header: `Net Productivity`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
      ...
      {
        accessorKey: 'material_name_id',
        header: `Material Name Id`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnProductivityList(); // Call refetch here
  ...
  const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnProductivity({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnProductivityUpdate: inpdata
      }).unwrap();

      toast.success('Productivity updated successfully');
      refetchTrnProductivityList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
          <ProductivityGridForm
            setOpen={setIsCreatePopupOpen}
            refetchList={refetchTrnProductivityList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this Productivity ?`}
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
          <MuiTanstackTable
            tableTitle=''
            columns={columns}
            importedData={fetchedTrnProductivityData ?? []}
            rowCount={rowCount}
...
export default ProductivityListing;
