...
import AssetDocumentsInlineForm from './AssetDocumentsInlineForm';
import AssetDocumentsGridForm from './AssetDocumentsGridForm';
import AssetDocumentsGlobalForm from './AssetDocumentsGlobalForm';
import {
...
const AssetDocumentsListing: React.FC<{
  ...
  const {
    data: fetchedTrnAssetDocumentData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchTrnAssetDocumentList
  } = useGetAllTrnAssetDocumentsTrnAssetDocumentGetQuery(
    {
      ...tenantIDHeader()
      //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedTrnAssetDocumentData) {
      setIsDataReady(true);
      setRowCount(fetchedTrnAssetDocumentData.meta?.total_items);
    }
  }, [fetchedTrnAssetDocumentData, isLoadingData, isLoadingError]);
          ...
          return (
            <div className='flex items-center'>
              <div
                ...
                onClick={() =>
                  router.push(
                    `/dashboard/asset-management/asset-documents/${row.original.id}`
                  )
                }
              >
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnAssetDocumentList(); // Call refetch here
  ...
  const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnAssetDocument({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnAssetDocumentUpdate: inpdata
      }).unwrap();

      toast.success('Asset Documents updated successfully');
      refetchTrnAssetDocumentList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
          <AssetDocumentsGridForm
            setOpen={setIsCreatePopupOpen}
            refetchList={refetchTrnAssetDocumentList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this AssetDocuments ?`}
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
          <MuiTanstackTable
            tableTitle=''
            columns={columns}
            importedData={fetchedTrnAssetDocumentData ?? []}
            rowCount={rowCount}
...
export default AssetDocumentsListing;
