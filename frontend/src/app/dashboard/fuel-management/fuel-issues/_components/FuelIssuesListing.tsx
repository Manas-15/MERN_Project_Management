...
import FuelIssuesInlineForm from './FuelIssuesInlineForm';
import FuelIssuesGridForm from './FuelIssuesGridForm';
import FuelIssuesGlobalForm from './FuelIssuesGlobalForm';
import {
...
const FuelIssuesListing: React.FC<{
  ...
  const {
    data: fetchedTrnFsFuelIssueData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchTrnFsFuelIssueList
  } = useGetAllTrnFsFuelIssuesTrnFsFuelIssueGetQuery(
    {
      ...tenantIDHeader()
      //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedTrnFsFuelIssueData) {
      setIsDataReady(true);
      setRowCount(fetchedTrnFsFuelIssueData.meta?.total_items);
    }
  }, [fetchedTrnFsFuelIssueData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
    () => [
          ...
          return (
            <div className='flex items-center'>
              <div
                ...
                onClick={() =>
                  router.push(
                    `/dashboard/fuel-management/fuel-issues/${row.original.id}`
                  )
                }
              >
      ...
      {
        accessorKey: 'fuel_receiver_id',
        header: `Fuel Receiver Id`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
      ...
      {
        accessorKey: 'fuel_receiver_name',
        header: `Fuel Receiver Name`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
        router.push(`/dashboard/fuel-management/fuel-issues/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnFsFuelIssueList(); // Call refetch here
  ...
  const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnFsFuelIssue({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnFsFuelIssueUpdate: inpdata
      }).unwrap();

      toast.success('Fuel Issues updated successfully');
      refetchTrnFsFuelIssueList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
          <FuelIssuesGridForm
            ...
            refetchList={refetchTrnFsFuelIssueList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this FuelIssues ?`}
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
          <MuiTanstackTable
            tableTitle=''
            columns={columns}
            importedData={fetchedTrnFsFuelIssueData ?? []}
            rowCount={rowCount}
...
export default FuelIssuesListing;
