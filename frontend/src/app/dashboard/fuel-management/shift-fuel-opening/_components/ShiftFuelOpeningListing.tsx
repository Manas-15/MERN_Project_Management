...
import ShiftFuelOpeningInlineForm from './ShiftFuelOpeningInlineForm';
import ShiftFuelOpeningGridForm from './ShiftFuelOpeningGridForm';
import ShiftFuelOpeningGlobalForm from './ShiftFuelOpeningGlobalForm';
import {
...
const ShiftFuelOpeningListing: React.FC<{
  ...
  const {
    data: fetchedTrnShiftFuelOpeningData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchTrnShiftFuelOpeningList
  } = useGetAllTrnShiftFuelOpeningsTrnShiftFuelOpeningGetQuery(
    {
      ...tenantIDHeader()
      //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedTrnShiftFuelOpeningData) {
      setIsDataReady(true);
      setRowCount(fetchedTrnShiftFuelOpeningData.meta?.total_items);
    }
  }, [fetchedTrnShiftFuelOpeningData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
    () => [
          ...
          return (
            <div className='flex items-center'>
              <div
                ...
                onClick={() =>
                  router.push(
                    `/dashboard/fuel-management/shift-fuel-opening/${row.original.id}`
                  )
                }
              >
      ...
      {
        accessorKey: 'previous_opening_balance',
        header: `Previous Opening Balance`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnShiftFuelOpeningList(); // Call refetch here
  ...
  const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnShiftFuelOpening({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnShiftFuelOpeningUpdate: inpdata
      }).unwrap();

      toast.success('Shift Fuel Opening updated successfully');
      refetchTrnShiftFuelOpeningList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
          <ShiftFuelOpeningGridForm
            setOpen={setIsCreatePopupOpen}
            refetchList={refetchTrnShiftFuelOpeningList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this ShiftFuelOpening ?`}
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
          <MuiTanstackTable
            tableTitle=''
            columns={columns}
            importedData={fetchedTrnShiftFuelOpeningData ?? []}
            rowCount={rowCount}
...
export default ShiftFuelOpeningListing;
