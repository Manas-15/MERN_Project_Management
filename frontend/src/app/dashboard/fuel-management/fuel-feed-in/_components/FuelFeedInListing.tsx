...
import FuelFeedInInlineForm from './FuelFeedInInlineForm';
import FuelFeedInGridForm from './FuelFeedInGridForm';
import FuelFeedInGlobalForm from './FuelFeedInGlobalForm';
import {
...
const FuelFeedInListing: React.FC<{
  ...
  const {
    data: fetchedTrnFuelFeedinData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchTrnFuelFeedinList
  } = useGetAllTrnFuelFeedinsTrnFuelFeedinGetQuery(
    {
      ...tenantIDHeader()
      //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedTrnFuelFeedinData) {
      setIsDataReady(true);
      setRowCount(fetchedTrnFuelFeedinData.meta?.total_items);
    }
  }, [fetchedTrnFuelFeedinData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
    () => [
          ...
          return (
            <div className='flex items-center'>
              <div
                ...
                onClick={() =>
                  router.push(
                    `/dashboard/fuel-management/fuel-feed-in/${row.original.id}`
                  )
                }
              >
      ...
      {
        accessorKey: 'fulefeedin_station',
        header: `Fulefeedin Station`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
      ...
      {
        accessorKey: 'fule_station_name',
        header: `Fule Station Name`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
      ...
      {
        accessorKey: 'other_station_detail',
        header: `Other Station Detail`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
      ...
      {
        accessorKey: 'fuel_tank_status',
        header: `Fuel Tank Status`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
      ...
      {
        accessorKey: 'attach_invoice',
        header: `Attach Invoice`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnFuelFeedinList(); // Call refetch here
  ...
  const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnFuelFeedin({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnFuelFeedinUpdate: inpdata
      }).unwrap();

      toast.success('Fuel Feed In updated successfully');
      refetchTrnFuelFeedinList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
          <FuelFeedInGridForm
            ...
            refetchList={refetchTrnFuelFeedinList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this FuelFeedIn ?`}
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
          <MuiTanstackTable
            tableTitle=''
            columns={columns}
            importedData={fetchedTrnFuelFeedinData ?? []}
            rowCount={rowCount}
...
export default FuelFeedInListing;
