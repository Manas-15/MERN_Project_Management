...
import FuelTransfersInlineForm from './FuelTransfersInlineForm';
import FuelTransfersGridForm from './FuelTransfersGridForm';
import FuelTransfersGlobalForm from './FuelTransfersGlobalForm';
import {
...
const FuelTransfersListing: React.FC<{
  ...
  const {
    data: fetchedTrnFsFuelTransferData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchTrnFsFuelTransferList
  } = useGetAllTrnFsFuelTransfersTrnFsFuelTransferGetQuery(
    {
      ...tenantIDHeader()
      //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedTrnFsFuelTransferData) {
      setIsDataReady(true);
      setRowCount(fetchedTrnFsFuelTransferData.meta?.total_items);
    }
  }, [fetchedTrnFsFuelTransferData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
    () => [
          ...
          return (
            <div className='flex items-center'>
              <div
                ...
                onClick={() =>
                  router.push(
                    `/dashboard/fuel-management/fuel-transfers/${row.original.id}`
                  )
                }
              >
      ...
      {
        accessorKey: 'to_fuel_station_name',
        header: `To Fuel Station Name`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnFsFuelTransferList(); // Call refetch here
  ...
  const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnFsFuelTransfer({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnFsFuelTransferUpdate: inpdata
      }).unwrap();

      toast.success('Fuel Transfers updated successfully');
      refetchTrnFsFuelTransferList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
          <FuelTransfersGridForm
            setOpen={setIsCreatePopupOpen}
            refetchList={refetchTrnFsFuelTransferList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this FuelTransfers ?`}
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
          <MuiTanstackTable
            tableTitle=''
            columns={columns}
            importedData={fetchedTrnFsFuelTransferData ?? []}
            rowCount={rowCount}
...
export default FuelTransfersListing;
