...
const formSchema = z.object({
  ...
  fuel_vendor_id: z.number().min(1, {
    message: 'Fuel Vendor Id is required'
  }),
...
export default function FuelReceiptsGlobalForm({
  setOpen,
  refetchList,
  selectedItem,
  setSelectedItem,
  ...
  const { data: fetchedRefVendorData, refetch: refetchRefVendorList } =
    useGetAllRefVendorsRefVendorGetQuery(
      {
        ...tenantIDHeader()
      },
  ...
  const defaultValues = {
    ...
    fuel_vendor_id: selectedItem?.fuel_vendor_id || 0,
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    if (selectedItem) {
      ...
      try {
        await createTrnFsFuelReceive({
          ...tenantIDHeader(),
          trnFsFuelReceiveInsert: values
        }).unwrap();
        toast.success(`Fuel Receipts created successfully`);
        afterSubmit();
      } catch (error) {
        toast.error('Failed to create Fuel Receipts');
      }
    }
  }
            ...
            <CardContent className='form-dynamic-height overflow-y-auto px-6'>
              ...
              <div className='mb-6 grid grid-cols-1 gap-6'>
                <Controller
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Fuel Vendor Id</FormLabel>
                      <MultiSelectSearchDropdown
                        {...field} // Spread the form field props
                        keyName='fuel_vendor_id'
                        isSearchable={true}
                        multiSelect={false}
                        placeholder='Select...'
                        control={form.control} // Control passed from react-hook-form
