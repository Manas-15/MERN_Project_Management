...
import EntityWorkflowsInlineForm from './EntityWorkflowsInlineForm';
import EntityWorkflowsGridForm from './EntityWorkflowsGridForm';
import EntityWorkflowsGlobalForm from './EntityWorkflowsGlobalForm';
import { 
...
const EntityWorkflowsListing : React.FC<{
  ...
  //FIXME
  const [updateTrnEntityWorkflow] = useUpdateTrnEntityWorkflowTrnEntityWorkflowIdPutMutation();
  ...
  useEffect(() => {
   ...
   const {
    data: fetchedTrnEntityWorkflowData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchTrnEntityWorkflowList
  } = useGetAllTrnEntityWorkflowsTrnEntityWorkflowGetQuery(
    {
      ...tenantIDHeader(),
     //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedTrnEntityWorkflowData) {
      setIsDataReady(true);
      setRowCount(fetchedTrnEntityWorkflowData.meta?.total_items);
    }
  }, [fetchedTrnEntityWorkflowData, isLoadingData, isLoadingError]);
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/dashboard/workflow/entity-workflows/${row.original.id}`)}
                      >
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/dashboard/workflow/entity-workflows/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnEntityWorkflowList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnEntityWorkflow({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnEntityWorkflowUpdate: inpdata
      }).unwrap();

      toast.success('Entity Workflows updated successfully');
      refetchTrnEntityWorkflowList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <EntityWorkflowsGridForm
              setOpen={setIsCreatePopupOpen}
              refetchList={refetchTrnEntityWorkflowList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this EntityWorkflows ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedTrnEntityWorkflowData ?? []}
            rowCount={rowCount}
...
export default EntityWorkflowsListing;