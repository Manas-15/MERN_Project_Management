...
import FlexibleFieldsInlineForm from './FlexibleFieldsInlineForm';
import FlexibleFieldsGridForm from './FlexibleFieldsGridForm';
import FlexibleFieldsGlobalForm from './FlexibleFieldsGlobalForm';
import {
...
const FlexibleFieldsListing: React.FC<{
  ...
  const {
    data: fetchedSaasFlexFieldsData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchSaasFlexFieldsList
  } = useGetAllSaasFlexFieldssSaasFlexFieldsGetQuery(
    {
      ...tenantIDHeader()
      //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedSaasFlexFieldsData) {
      setIsDataReady(true);
      setRowCount(fetchedSaasFlexFieldsData.meta?.total_items);
    }
  }, [fetchedSaasFlexFieldsData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
    () => [
          ...
          return (
            <div className='flex items-center'>
              <div
                ...
                onClick={() =>
                  router.push(
                    `/dashboard/audit-monitoring/flexible-fields/${row.original.id}`
                  )
                }
              >
      ...
      {
        accessorKey: 'flex_feild_details',
        header: `Flex Feild Details`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
      ...
      {
        accessorKey: 'entity_applied',
        header: `Entity Applied`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchSaasFlexFieldsList(); // Call refetch here
  ...
  const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateSaasFlexFields({
        ...tenantIDHeader(),
        id: inpdata.id,
        saasFlexFieldsUpdate: inpdata
      }).unwrap();

      toast.success('Flexible Fields updated successfully');
      refetchSaasFlexFieldsList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
          <FlexibleFieldsGridForm
            setOpen={setIsCreatePopupOpen}
            refetchList={refetchSaasFlexFieldsList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this FlexibleFields ?`}
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
          <MuiTanstackTable
            tableTitle=''
            columns={columns}
            importedData={fetchedSaasFlexFieldsData ?? []}
            rowCount={rowCount}
...
export default FlexibleFieldsListing;
