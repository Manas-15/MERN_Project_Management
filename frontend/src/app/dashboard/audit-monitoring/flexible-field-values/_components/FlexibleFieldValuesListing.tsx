...
import FlexibleFieldValuesInlineForm from './FlexibleFieldValuesInlineForm';
import FlexibleFieldValuesGridForm from './FlexibleFieldValuesGridForm';
import FlexibleFieldValuesGlobalForm from './FlexibleFieldValuesGlobalForm';
import {
...
const FlexibleFieldValuesListing: React.FC<{
  ...
  const {
    data: fetchedSaasFlexFieldValuesData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchSaasFlexFieldValuesList
  } = useGetAllSaasFlexFieldValuessSaasFlexFieldValuesGetQuery(
    {
      ...tenantIDHeader()
      //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedSaasFlexFieldValuesData) {
      setIsDataReady(true);
      setRowCount(fetchedSaasFlexFieldValuesData.meta?.total_items);
    }
  }, [fetchedSaasFlexFieldValuesData, isLoadingData, isLoadingError]);
          ...
          return (
            <div className='flex items-center'>
              <div
                ...
                onClick={() =>
                  router.push(
                    `/dashboard/audit-monitoring/flexible-field-values/${row.original.id}`
                  )
                }
              >
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchSaasFlexFieldValuesList(); // Call refetch here
  ...
  const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateSaasFlexFieldValues({
        ...tenantIDHeader(),
        id: inpdata.id,
        saasFlexFieldValuesUpdate: inpdata
      }).unwrap();

      toast.success('Flexible Field Values updated successfully');
      refetchSaasFlexFieldValuesList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
          <FlexibleFieldValuesGridForm
            setOpen={setIsCreatePopupOpen}
            refetchList={refetchSaasFlexFieldValuesList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this FlexibleFieldValues ?`}
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
          <MuiTanstackTable
            tableTitle=''
            columns={columns}
            importedData={fetchedSaasFlexFieldValuesData ?? []}
            rowCount={rowCount}
...
export default FlexibleFieldValuesListing;
