...
import ApiKeyAuditInlineForm from './ApiKeyAuditInlineForm';
import ApiKeyAuditGridForm from './ApiKeyAuditGridForm';
import ApiKeyAuditGlobalForm from './ApiKeyAuditGlobalForm';
import {
...
const ApiKeyAuditListing: React.FC<{
  ...
  const {
    data: fetchedSaasApiKeyAuditData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchSaasApiKeyAuditList
  } = useGetAllSaasApiKeyAuditsSaasApiKeyAuditGetQuery(
    {
      ...tenantIDHeader()
      //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedSaasApiKeyAuditData) {
      setIsDataReady(true);
      setRowCount(fetchedSaasApiKeyAuditData.meta?.total_items);
    }
  }, [fetchedSaasApiKeyAuditData, isLoadingData, isLoadingError]);

  //Info: The following data is used for the columns
  const columns = useMemo<any>(
    () => [
      {
        accessorKey: 'current_api_key',
        header: `Current Api Key`,
        size: 150,
        enableSorting: true,
          ...
          return (
            <div className='flex items-center'>
              <div
                ...
                onClick={() =>
                  router.push(
                    `/dashboard/audit-monitoring/api-key-audit/${row.original.id}`
                  )
                }
              >
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchSaasApiKeyAuditList(); // Call refetch here
  ...
  const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateSaasApiKeyAudit({
        ...tenantIDHeader(),
        id: inpdata.id,
        saasApiKeyAuditUpdate: inpdata
      }).unwrap();

      toast.success('Api Key Audit updated successfully');
      refetchSaasApiKeyAuditList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
          <ApiKeyAuditGridForm
            ...
            refetchList={refetchSaasApiKeyAuditList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this ApiKeyAudit ?`}
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
          <MuiTanstackTable
            tableTitle=''
            columns={columns}
            importedData={fetchedSaasApiKeyAuditData ?? []}
            rowCount={rowCount}
...
export default ApiKeyAuditListing;
