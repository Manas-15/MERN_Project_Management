...
import EmployeesInlineForm from './EmployeesInlineForm';
import EmployeesGridForm from './EmployeesGridForm';
import EmployeesGlobalForm from './EmployeesGlobalForm';
import {
...
const EmployeesListing: React.FC<{
  ...
  const {
    data: fetchedTrnHrEmployeeData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchTrnHrEmployeeList
  } = useGetAllTrnHrEmployeesTrnHrEmployeeGetQuery(
    {
      ...tenantIDHeader()
      //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedTrnHrEmployeeData) {
      setIsDataReady(true);
      setRowCount(fetchedTrnHrEmployeeData.meta?.total_items);
    }
  }, [fetchedTrnHrEmployeeData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
    () => [
          ...
          return (
            <div className='flex items-center'>
              <div
                ...
                onClick={() =>
                  router.push(
                    `/dashboard/employee-management/employees/${row.original.id}`
                  )
                }
              >
      ...
      {
        accessorKey: 'transition_site_id',
        header: `Transition Site Id`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
      ...
      {
        accessorKey: 'transition_site_name',
        header: `Transition Site Name`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnHrEmployeeList(); // Call refetch here
  ...
  const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnHrEmployee({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnHrEmployeeUpdate: inpdata
      }).unwrap();

      toast.success('Employees updated successfully');
      refetchTrnHrEmployeeList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
          <EmployeesGridForm
            ...
            refetchList={refetchTrnHrEmployeeList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this Employees ?`}
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
          <MuiTanstackTable
            tableTitle=''
            columns={columns}
            importedData={fetchedTrnHrEmployeeData ?? []}
            rowCount={rowCount}
