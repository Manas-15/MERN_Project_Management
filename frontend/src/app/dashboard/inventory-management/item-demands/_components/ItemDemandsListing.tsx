...
import ItemDemandsInlineForm from './ItemDemandsInlineForm';
import ItemDemandsGridForm from './ItemDemandsGridForm';
import ItemDemandsGlobalForm from './ItemDemandsGlobalForm';
import { 
...
const ItemDemandsListing : React.FC<{
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedTrnInvItemDemandData) {
      setIsDataReady(true);
      setRowCount(fetchedTrnInvItemDemandData.meta?.total_items);
    }
  }, [fetchedTrnInvItemDemandData, isLoadingData, isLoadingError]);
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/dashboard/inventory-management/item-demands/${row.original.id}`)}
                      >
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/dashboard/inventory-management/item-demands/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnInvItemDemandList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnInvItemDemand({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnInvItemDemandUpdate: inpdata
      }).unwrap();

      toast.success('Item Demands updated successfully');
      refetchTrnInvItemDemandList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <ItemDemandsGridForm
              ...
              refetchList={refetchTrnInvItemDemandList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this ItemDemands ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedTrnInvItemDemandData ?? []}
            rowCount={rowCount}
...
export default ItemDemandsListing;