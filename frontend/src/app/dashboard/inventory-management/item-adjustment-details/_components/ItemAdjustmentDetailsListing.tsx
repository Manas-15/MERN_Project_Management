...
import ItemAdjustmentDetailsInlineForm from './ItemAdjustmentDetailsInlineForm';
import ItemAdjustmentDetailsGridForm from './ItemAdjustmentDetailsGridForm';
import ItemAdjustmentDetailsGlobalForm from './ItemAdjustmentDetailsGlobalForm';
import {
...
const ItemAdjustmentDetailsListing: React.FC<{
  ...
  const {
    data: fetchedTrnInvItemAdjustmentDtlData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchTrnInvItemAdjustmentDtlList
  } = useGetAllTrnInvItemAdjustmentDtlsTrnInvItemAdjustmentDtlGetQuery(
    {
      ...tenantIDHeader()
      //orderBy: orderBy,
  ...
  useEffect(() => {
    if (
      !isLoadingData &&
      !isLoadingError &&
      fetchedTrnInvItemAdjustmentDtlData
    ) {
      setIsDataReady(true);
      setRowCount(fetchedTrnInvItemAdjustmentDtlData.meta?.total_items);
    }
  }, [fetchedTrnInvItemAdjustmentDtlData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
    () => [
          ...
          return (
            <div className='flex items-center'>
              <div
                ...
                onClick={() =>
                  router.push(
                    `/dashboard/inventory-management/item-adjustment-details/${row.original.id}`
                  )
                }
              >
      ...
      {
        accessorKey: 'less_quantity',
        header: `Less Quantity`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnInvItemAdjustmentDtlList(); // Call refetch here
  ...
  const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnInvItemAdjustmentDtl({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnInvItemAdjustmentDtlUpdate: inpdata
      }).unwrap();

      toast.success('Item Adjustment Details updated successfully');
      refetchTrnInvItemAdjustmentDtlList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
          <ItemAdjustmentDetailsGridForm
            setOpen={setIsCreatePopupOpen}
            refetchList={refetchTrnInvItemAdjustmentDtlList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this ItemAdjustmentDetails ?`}
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
          <MuiTanstackTable
            tableTitle=''
            columns={columns}
            importedData={fetchedTrnInvItemAdjustmentDtlData ?? []}
            rowCount={rowCount}
...
export default ItemAdjustmentDetailsListing;
