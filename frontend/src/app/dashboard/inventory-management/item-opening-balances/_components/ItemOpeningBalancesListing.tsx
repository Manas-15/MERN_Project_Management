...
import ItemOpeningBalancesInlineForm from './ItemOpeningBalancesInlineForm';
import ItemOpeningBalancesGridForm from './ItemOpeningBalancesGridForm';
import ItemOpeningBalancesGlobalForm from './ItemOpeningBalancesGlobalForm';
import {
...
const ItemOpeningBalancesListing: React.FC<{
  ...
  const [updateItemOpeningBalance] =
    useUpdateItemOpeningBalanceItemOpeningBalanceIdPutMutation();
  const [deleteItemOpeningBalance] =
    useDeleteItemOpeningBalanceItemOpeningBalanceIdDeleteMutation();
  ...
  const {
    data: fetchedItemOpeningBalanceData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchItemOpeningBalanceList
  } = useGetAllItemOpeningBalancesItemOpeningBalanceGetQuery(
    {
      ...tenantIDHeader()
      //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedItemOpeningBalanceData) {
      setIsDataReady(true);
      setRowCount(fetchedItemOpeningBalanceData.meta?.total_items);
    }
  }, [fetchedItemOpeningBalanceData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
    () => [
          ...
          return (
            <div className='flex items-center'>
              <div
                ...
                onClick={() =>
                  router.push(
                    `/dashboard/inventory-management/item-opening-balances/${row.original.id}`
                  )
                }
              >
      ...
      {
        accessorKey: 'item_unit_rate',
        header: `Item Unit Rate`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchItemOpeningBalanceList(); // Call refetch here
  ...
  const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateItemOpeningBalance({
        ...tenantIDHeader(),
        id: inpdata.id,
        itemOpeningBalanceUpdate: inpdata
      }).unwrap();

      toast.success('Item Opening Balances updated successfully');
      refetchItemOpeningBalanceList();
    } catch (error) {
    ...
    <>
      {isCreatePopupOpen && (
        <Modal
          title={
            selectedItem
              ? 'Edit Item Opening Balances'
              : 'Create Item Opening Balances'
          }
          description=''
        ...
        >
          <ItemOpeningBalancesGridForm
            setOpen={setIsCreatePopupOpen}
            refetchList={refetchItemOpeningBalanceList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this ItemOpeningBalances ?`}
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
          <MuiTanstackTable
            tableTitle=''
            columns={columns}
            importedData={fetchedItemOpeningBalanceData ?? []}
            rowCount={rowCount}
...
export default ItemOpeningBalancesListing;
