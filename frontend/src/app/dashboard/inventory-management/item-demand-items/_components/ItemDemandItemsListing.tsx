...
import ItemDemandItemsInlineForm from './ItemDemandItemsInlineForm';
import ItemDemandItemsGridForm from './ItemDemandItemsGridForm';
import ItemDemandItemsGlobalForm from './ItemDemandItemsGlobalForm';
import { 
...
const ItemDemandItemsListing : React.FC<{
  ...
  useEffect(() => {
   ...
   const {
    data: fetchedTrnInvItemDemandItemsData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchTrnInvItemDemandItemsList
  } = useGetAllTrnInvItemDemandItemssTrnInvItemDemandItemsGetQuery(
    {
      ...tenantIDHeader(),
     //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedTrnInvItemDemandItemsData) {
      setIsDataReady(true);
      setRowCount(fetchedTrnInvItemDemandItemsData.meta?.total_items);
    }
  }, [fetchedTrnInvItemDemandItemsData, isLoadingData, isLoadingError]);
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/dashboard/inventory-management/item-demand-items/${row.original.id}`)}
                      >
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/dashboard/inventory-management/item-demand-items/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnInvItemDemandItemsList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnInvItemDemandItems({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnInvItemDemandItemsUpdate: inpdata
      }).unwrap();

      toast.success('Item Demand Items updated successfully');
      refetchTrnInvItemDemandItemsList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <ItemDemandItemsGridForm
              setOpen={setIsCreatePopupOpen}
              refetchList={refetchTrnInvItemDemandItemsList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this ItemDemandItems ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedTrnInvItemDemandItemsData ?? []}
            rowCount={rowCount}
...
export default ItemDemandItemsListing;