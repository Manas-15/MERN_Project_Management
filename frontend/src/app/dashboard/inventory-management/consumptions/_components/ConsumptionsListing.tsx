...
import ConsumptionsInlineForm from './ConsumptionsInlineForm';
import ConsumptionsGridForm from './ConsumptionsGridForm';
import ConsumptionsGlobalForm from './ConsumptionsGlobalForm';
import {
...
const ConsumptionsListing: React.FC<{
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedTrnInvConsumptionData) {
      setIsDataReady(true);
      setRowCount(fetchedTrnInvConsumptionData.meta?.total_items);
    }
  }, [fetchedTrnInvConsumptionData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
    () => [
          ...
          return (
            <div className='flex items-center'>
              <div
                ...
                onClick={() =>
                  router.push(
                    `/dashboard/inventory-management/consumptions/${row.original.id}`
                  )
                }
              >
      ...
      {
        accessorKey: 'consumption_for',
        header: `Consumption For`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
      ...
      {
        accessorKey: 'update_item_config',
        header: `Update Item Config`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnInvConsumptionList(); // Call refetch here
  ...
  const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnInvConsumption({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnInvConsumptionUpdate: inpdata
      }).unwrap();

      toast.success('Consumptions updated successfully');
      refetchTrnInvConsumptionList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
          <ConsumptionsGridForm
            setOpen={setIsCreatePopupOpen}
            refetchList={refetchTrnInvConsumptionList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this Consumptions ?`}
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
          <MuiTanstackTable
            tableTitle=''
            columns={columns}
            importedData={fetchedTrnInvConsumptionData ?? []}
            rowCount={rowCount}
...
export default ConsumptionsListing;
