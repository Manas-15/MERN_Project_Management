...
import ItemTransferDetailsInlineForm from './ItemTransferDetailsInlineForm';
import ItemTransferDetailsGridForm from './ItemTransferDetailsGridForm';
import ItemTransferDetailsGlobalForm from './ItemTransferDetailsGlobalForm';
import { 
...
const ItemTransferDetailsListing : React.FC<{
  ...
  useEffect(() => {
   ...
   const {
    data: fetchedTrnInvItemTransferDtlData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchTrnInvItemTransferDtlList
  } = useGetAllTrnInvItemTransferDtlsTrnInvItemTransferDtlGetQuery(
    {
      ...tenantIDHeader(),
     //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedTrnInvItemTransferDtlData) {
      setIsDataReady(true);
      setRowCount(fetchedTrnInvItemTransferDtlData.meta?.total_items);
    }
  }, [fetchedTrnInvItemTransferDtlData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
      () => [
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/dashboard/inventory-management/item-transfer-details/${row.original.id}`)}
                      >
          ...
          {
              accessorKey: 'demand_prending_qty',
              header: `Demand Prending Qty`,
              size: 150,
              enableSorting: true,
              enableColumnFilter: false,
              enableEditing: true,
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/dashboard/inventory-management/item-transfer-details/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnInvItemTransferDtlList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnInvItemTransferDtl({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnInvItemTransferDtlUpdate: inpdata
      }).unwrap();

      toast.success('Item Transfer Details updated successfully');
      refetchTrnInvItemTransferDtlList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <ItemTransferDetailsGridForm
              setOpen={setIsCreatePopupOpen}
              refetchList={refetchTrnInvItemTransferDtlList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this ItemTransferDetails ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedTrnInvItemTransferDtlData ?? []}
            rowCount={rowCount}
...
export default ItemTransferDetailsListing;