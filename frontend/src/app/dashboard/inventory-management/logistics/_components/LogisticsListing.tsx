...
import LogisticsInlineForm from './LogisticsInlineForm';
import LogisticsGridForm from './LogisticsGridForm';
import LogisticsGlobalForm from './LogisticsGlobalForm';
import { 
...
const LogisticsListing : React.FC<{
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedTrnInvLogisticsData) {
      setIsDataReady(true);
      setRowCount(fetchedTrnInvLogisticsData.meta?.total_items);
    }
  }, [fetchedTrnInvLogisticsData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
      () => [
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/dashboard/inventory-management/logistics/${row.original.id}`)}
                      >
          ...
          {
              accessorKey: 'devision_name',
              header: `Devision Name`,
              size: 150,
              enableSorting: true,
              enableColumnFilter: false,
              enableEditing: true,
          ...
          {
              accessorKey: 'origin_cost_center',
              header: `Origin Cost Center`,
              size: 150,
              enableSorting: true,
              enableColumnFilter: false,
              enableEditing: true,
          ...
          {
              accessorKey: 'destination_cost_center',
              header: `Destination Cost Center`,
              size: 150,
              enableSorting: true,
              enableColumnFilter: false,
              enableEditing: true,
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/dashboard/inventory-management/logistics/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnInvLogisticsList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnInvLogistics({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnInvLogisticsUpdate: inpdata
      }).unwrap();

      toast.success('Logistics updated successfully');
      refetchTrnInvLogisticsList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <LogisticsGridForm
              ...
              refetchList={refetchTrnInvLogisticsList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this Logistics ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedTrnInvLogisticsData ?? []}
            rowCount={rowCount}
