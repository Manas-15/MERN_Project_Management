...
import ConsumptionItemsInlineForm from './ConsumptionItemsInlineForm';
import ConsumptionItemsGridForm from './ConsumptionItemsGridForm';
import ConsumptionItemsGlobalForm from './ConsumptionItemsGlobalForm';
import {
...
const ConsumptionItemsListing: React.FC<{
  ...
  const {
    data: fetchedTrnInvConsumptionItemsData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchTrnInvConsumptionItemsList
  } = useGetAllTrnInvConsumptionItemssTrnInvConsumptionItemsGetQuery(
    {
      ...tenantIDHeader()
      //orderBy: orderBy,
  ...
  useEffect(() => {
    if (
      !isLoadingData &&
      !isLoadingError &&
      fetchedTrnInvConsumptionItemsData
    ) {
      setIsDataReady(true);
      setRowCount(fetchedTrnInvConsumptionItemsData.meta?.total_items);
    }
  }, [fetchedTrnInvConsumptionItemsData, isLoadingData, isLoadingError]);

  //Info: The following data is used for the columns
  const columns = useMemo<any>(
    () => [
      {
        accessorKey: 'consumption_id',
          ...
          return (
            <div className='flex items-center'>
              <div
                ...
                onClick={() =>
                  router.push(
                    `/dashboard/inventory-management/consumption-items/${row.original.id}`
                  )
                }
              >
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnInvConsumptionItemsList(); // Call refetch here
  ...
  const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnInvConsumptionItems({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnInvConsumptionItemsUpdate: inpdata
      }).unwrap();

      toast.success('Consumption Items updated successfully');
      refetchTrnInvConsumptionItemsList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
          <ConsumptionItemsGridForm
            setOpen={setIsCreatePopupOpen}
            refetchList={refetchTrnInvConsumptionItemsList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this ConsumptionItems ?`}
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
          <MuiTanstackTable
            tableTitle=''
            columns={columns}
            importedData={fetchedTrnInvConsumptionItemsData ?? []}
            rowCount={rowCount}
...
export default ConsumptionItemsListing;
