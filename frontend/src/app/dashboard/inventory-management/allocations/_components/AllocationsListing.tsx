...
import AllocationsInlineForm from './AllocationsInlineForm';
import AllocationsGridForm from './AllocationsGridForm';
import AllocationsGlobalForm from './AllocationsGlobalForm';
import {
...
const AllocationsListing: React.FC<{
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedTrnInvAllocationData) {
      setIsDataReady(true);
      setRowCount(fetchedTrnInvAllocationData.meta?.total_items);
    }
  }, [fetchedTrnInvAllocationData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
    () => [
          ...
          return (
            <div className='flex items-center'>
              <div
                ...
                onClick={() =>
                  router.push(
                    `/dashboard/inventory-management/allocations/${row.original.id}`
                  )
                }
              >
      ...
      {
        accessorKey: 'allocation_for',
        header: `Allocation For`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnInvAllocationList(); // Call refetch here
  ...
  const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnInvAllocation({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnInvAllocationUpdate: inpdata
      }).unwrap();

      toast.success('Allocations updated successfully');
      refetchTrnInvAllocationList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
          <AllocationsGridForm
            ...
            refetchList={refetchTrnInvAllocationList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this Allocations ?`}
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
          <MuiTanstackTable
            tableTitle=''
            columns={columns}
            importedData={fetchedTrnInvAllocationData ?? []}
            rowCount={rowCount}
...
export default AllocationsListing;
