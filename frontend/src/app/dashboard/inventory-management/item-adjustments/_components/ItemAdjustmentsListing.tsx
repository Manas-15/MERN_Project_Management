...
import ItemAdjustmentsInlineForm from './ItemAdjustmentsInlineForm';
import ItemAdjustmentsGridForm from './ItemAdjustmentsGridForm';
import ItemAdjustmentsGlobalForm from './ItemAdjustmentsGlobalForm';
import {
...
const ItemAdjustmentsListing: React.FC<{
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedTrnInvItemAdjustmentData) {
      setIsDataReady(true);
      setRowCount(fetchedTrnInvItemAdjustmentData.meta?.total_items);
    }
  }, [fetchedTrnInvItemAdjustmentData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
    () => [
          ...
          return (
            <div className='flex items-center'>
              <div
                ...
                onClick={() =>
                  router.push(
                    `/dashboard/inventory-management/item-adjustments/${row.original.id}`
                  )
                }
              >
      ...
      {
        accessorKey: 'adjustment_for',
        header: `Adjustment For`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
      ...
      {
        accessorKey: 'total_add_quantity',
        header: `Total Add Quantity`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
      ...
      {
        accessorKey: 'total_less_quantity',
        header: `Total Less Quantity`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnInvItemAdjustmentList(); // Call refetch here
  ...
  const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnInvItemAdjustment({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnInvItemAdjustmentUpdate: inpdata
      }).unwrap();

      toast.success('Item Adjustments updated successfully');
      refetchTrnInvItemAdjustmentList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
          <ItemAdjustmentsGridForm
            setOpen={setIsCreatePopupOpen}
            refetchList={refetchTrnInvItemAdjustmentList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this ItemAdjustments ?`}
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
          <MuiTanstackTable
            tableTitle=''
            columns={columns}
            importedData={fetchedTrnInvItemAdjustmentData ?? []}
            rowCount={rowCount}
...
export default ItemAdjustmentsListing;
