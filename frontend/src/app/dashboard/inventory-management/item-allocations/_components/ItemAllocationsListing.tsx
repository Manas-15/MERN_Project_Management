...
import ItemAllocationsInlineForm from './ItemAllocationsInlineForm';
import ItemAllocationsGridForm from './ItemAllocationsGridForm';
import ItemAllocationsGlobalForm from './ItemAllocationsGlobalForm';
import {
...
const ItemAllocationsListing: React.FC<{
  ...
  const {
    data: fetchedTrnInvItemAllocationData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchTrnInvItemAllocationList
  } = useGetAllTrnInvItemAllocationsTrnInvItemAllocationGetQuery(
    {
      ...tenantIDHeader()
      //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedTrnInvItemAllocationData) {
      setIsDataReady(true);
      setRowCount(fetchedTrnInvItemAllocationData.meta?.total_items);
    }
  }, [fetchedTrnInvItemAllocationData, isLoadingData, isLoadingError]);
          ...
          return (
            <div className='flex items-center'>
              <div
                ...
                onClick={() =>
                  router.push(
                    `/dashboard/inventory-management/item-allocations/${row.original.id}`
                  )
                }
              >
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnInvItemAllocationList(); // Call refetch here
  ...
  const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnInvItemAllocation({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnInvItemAllocationUpdate: inpdata
      }).unwrap();

      toast.success('Item Allocations updated successfully');
      refetchTrnInvItemAllocationList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
          <ItemAllocationsGridForm
            setOpen={setIsCreatePopupOpen}
            refetchList={refetchTrnInvItemAllocationList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this ItemAllocations ?`}
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
          <MuiTanstackTable
            tableTitle=''
            columns={columns}
            importedData={fetchedTrnInvItemAllocationData ?? []}
            rowCount={rowCount}
...
export default ItemAllocationsListing;
