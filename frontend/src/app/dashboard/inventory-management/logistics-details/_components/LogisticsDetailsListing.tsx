...
import LogisticsDetailsInlineForm from './LogisticsDetailsInlineForm';
import LogisticsDetailsGridForm from './LogisticsDetailsGridForm';
import LogisticsDetailsGlobalForm from './LogisticsDetailsGlobalForm';
import { 
...
const LogisticsDetailsListing : React.FC<{
  ...
  useEffect(() => {
   ...
   const {
    data: fetchedTrnInvLogisticsDtlData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchTrnInvLogisticsDtlList
  } = useGetAllTrnInvLogisticsDtlsTrnInvLogisticsDtlGetQuery(
    {
      ...tenantIDHeader(),
     //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedTrnInvLogisticsDtlData) {
      setIsDataReady(true);
      setRowCount(fetchedTrnInvLogisticsDtlData.meta?.total_items);
    }
  }, [fetchedTrnInvLogisticsDtlData, isLoadingData, isLoadingError]);
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/dashboard/inventory-management/logistics-details/${row.original.id}`)}
                      >
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/dashboard/inventory-management/logistics-details/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchTrnInvLogisticsDtlList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateTrnInvLogisticsDtl({
        ...tenantIDHeader(),
        id: inpdata.id,
        trnInvLogisticsDtlUpdate: inpdata
      }).unwrap();

      toast.success('Logistics Details updated successfully');
      refetchTrnInvLogisticsDtlList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <LogisticsDetailsGridForm
              setOpen={setIsCreatePopupOpen}
              refetchList={refetchTrnInvLogisticsDtlList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this LogisticsDetails ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedTrnInvLogisticsDtlData ?? []}
            rowCount={rowCount}
...
export default LogisticsDetailsListing;