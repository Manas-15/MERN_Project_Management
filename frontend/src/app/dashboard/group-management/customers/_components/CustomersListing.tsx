...
import CustomersInlineForm from './CustomersInlineForm';
import CustomersGridForm from './CustomersGridForm';
import CustomersGlobalForm from './CustomersGlobalForm';
import {
...
const CustomersListing: React.FC<{
  ...
  const {
    data: fetchedSaasCustomersData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchSaasCustomersList
  } = useGetAllSaasCustomerssSaasCustomersGetQuery(
    {
      ...tenantIDHeader()
      //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedSaasCustomersData) {
      setIsDataReady(true);
      setRowCount(fetchedSaasCustomersData.meta?.total_items);
    }
  }, [fetchedSaasCustomersData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
    () => [
          ...
          return (
            <div className='flex items-center'>
              <div
                ...
                onClick={() =>
                  router.push(
                    `/dashboard/group-management/customers/${row.original.id}`
                  )
                }
              >
      ...
      {
        accessorKey: 'customer_organization',
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
        router.push(`/dashboard/group-management/customers/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchSaasCustomersList(); // Call refetch here
  ...
  const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateSaasCustomers({
        ...tenantIDHeader(),
        id: inpdata.id,
        saasCustomersUpdate: inpdata
      }).unwrap();

      toast.success('Customers updated successfully');
      refetchSaasCustomersList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
          <CustomersGridForm
            ...
            refetchList={refetchSaasCustomersList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this Customers ?`}
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
          <MuiTanstackTable
            tableTitle=''
            columns={columns}
            importedData={fetchedSaasCustomersData ?? []}
            rowCount={rowCount}
