...
import FuelStationsInlineForm from './FuelStationsInlineForm';
import FuelStationsGridForm from './FuelStationsGridForm';
import FuelStationsGlobalForm from './FuelStationsGlobalForm';
import {
...
const FuelStationsListing: React.FC<{
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedRefFuelStationData) {
      setIsDataReady(true);
      setRowCount(fetchedRefFuelStationData.meta?.total_items);
    }
  }, [fetchedRefFuelStationData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
    () => [
          ...
          return (
            <div className='flex items-center'>
              <div
                ...
                onClick={() =>
                  router.push(
                    `/dashboard/group-management/fuel-stations/${row.original.id}`
                  )
                }
              >
      ...
      {
        accessorKey: 'fule_station_type',
        header: `Fule Station Type`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
      ...
      {
        accessorKey: 'all_sites_indicator',
        header: `All Sites Indicator`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchRefFuelStationList(); // Call refetch here
  ...
  const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateRefFuelStation({
        ...tenantIDHeader(),
        id: inpdata.id,
        refFuelStationUpdate: inpdata
      }).unwrap();

      toast.success('Fuel Stations updated successfully');
      refetchRefFuelStationList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
          <FuelStationsGridForm
            setOpen={setIsCreatePopupOpen}
            refetchList={refetchRefFuelStationList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this FuelStations ?`}
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
          <MuiTanstackTable
            tableTitle=''
            columns={columns}
            importedData={fetchedRefFuelStationData ?? []}
            rowCount={rowCount}
...
export default FuelStationsListing;
