...
import TaxManagementInlineForm from './TaxManagementInlineForm';
import TaxManagementGridForm from './TaxManagementGridForm';
import TaxManagementGlobalForm from './TaxManagementGlobalForm';
import {
...
const TaxManagementListing: React.FC<{
  ...
  const {
    data: fetchedRefTaxData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchRefTaxList
  } = useGetAllRefTaxsRefTaxGetQuery(
    {
      ...tenantIDHeader()
      //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedRefTaxData) {
      setIsDataReady(true);
      setRowCount(fetchedRefTaxData.meta?.total_items);
    }
  }, [fetchedRefTaxData, isLoadingData, isLoadingError]);
          ...
          return (
            <div className='flex items-center'>
              <div
                ...
                onClick={() =>
                  router.push(
                    `/dashboard/accounting-finance/tax-management/${row.original.id}`
                  )
                }
              >
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchRefTaxList(); // Call refetch here
  ...
  const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateRefTax({
        ...tenantIDHeader(),
        id: inpdata.id,
        refTaxUpdate: inpdata
      }).unwrap();

      toast.success('Tax Management updated successfully');
      refetchRefTaxList();
    } catch (error) {
      toast.error('Failed to update Tax Management');
    }
  };
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
          <TaxManagementGridForm
            setOpen={setIsCreatePopupOpen}
            refetchList={refetchRefTaxList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this TaxManagement ?`}
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
          <MuiTanstackTable
            tableTitle=''
            columns={columns}
            importedData={fetchedRefTaxData ?? []}
            rowCount={rowCount}
...
export default TaxManagementListing;
