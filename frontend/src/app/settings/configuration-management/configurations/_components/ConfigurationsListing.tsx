...
import ConfigurationsInlineForm from './ConfigurationsInlineForm';
import ConfigurationsGridForm from './ConfigurationsGridForm';
import ConfigurationsGlobalForm from './ConfigurationsGlobalForm';
import { 
...
const ConfigurationsListing : React.FC<{
  ...
  //FIXME
  const [updateSaasRefConfigurations] = useUpdateSaasRefConfigurationsSaasRefConfigurationsIdPutMutation();
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedSaasRefConfigurationsData) {
      setIsDataReady(true);
      setRowCount(fetchedSaasRefConfigurationsData.meta?.total_items);
    }
  }, [fetchedSaasRefConfigurationsData, isLoadingData, isLoadingError]);
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/settings/configuration-management/configurations/${row.original.id}`)}
                      >
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/settings/configuration-management/configurations/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchSaasRefConfigurationsList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateSaasRefConfigurations({
        ...tenantIDHeader(),
        id: inpdata.id,
        saasRefConfigurationsUpdate: inpdata
      }).unwrap();

      toast.success('Configurations updated successfully');
      refetchSaasRefConfigurationsList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <ConfigurationsGridForm
              setOpen={setIsCreatePopupOpen}
              refetchList={refetchSaasRefConfigurationsList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this Configurations ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedSaasRefConfigurationsData ?? []}
            rowCount={rowCount}
