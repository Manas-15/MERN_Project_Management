...
import TenantWorkflowsInlineForm from './TenantWorkflowsInlineForm';
import TenantWorkflowsGridForm from './TenantWorkflowsGridForm';
import TenantWorkflowsGlobalForm from './TenantWorkflowsGlobalForm';
import { 
...
const TenantWorkflowsListing : React.FC<{
  ...
  //FIXME
  const [updateSetTenantWorkflows] = useUpdateSetTenantWorkflowsSetTenantWorkflowsIdPutMutation();
  ...
  useEffect(() => {
   ...
   const {
    data: fetchedSetTenantWorkflowsData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchSetTenantWorkflowsList
  } = useGetAllSetTenantWorkflowssSetTenantWorkflowsGetQuery(
    {
      ...tenantIDHeader(),
     //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedSetTenantWorkflowsData) {
      setIsDataReady(true);
      setRowCount(fetchedSetTenantWorkflowsData.meta?.total_items);
    }
  }, [fetchedSetTenantWorkflowsData, isLoadingData, isLoadingError]);
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/settings/configuration-management/tenant-workflows/${row.original.id}`)}
                      >
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/settings/configuration-management/tenant-workflows/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchSetTenantWorkflowsList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateSetTenantWorkflows({
        ...tenantIDHeader(),
        id: inpdata.id,
        setTenantWorkflowsUpdate: inpdata
      }).unwrap();

      toast.success('Tenant Workflows updated successfully');
      refetchSetTenantWorkflowsList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <TenantWorkflowsGridForm
              setOpen={setIsCreatePopupOpen}
              refetchList={refetchSetTenantWorkflowsList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this TenantWorkflows ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedSetTenantWorkflowsData ?? []}
            rowCount={rowCount}
...
export default TenantWorkflowsListing;