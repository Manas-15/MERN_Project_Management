...
import DocumentsInlineForm from './DocumentsInlineForm';
import DocumentsGridForm from './DocumentsGridForm';
import DocumentsGlobalForm from './DocumentsGlobalForm';
import { 
...
const DocumentsListing : React.FC<{
  ...
  useEffect(() => {
   ...
   const {
    data: fetchedSaasRefDocumentsData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchSaasRefDocumentsList
  } = useGetAllSaasRefDocumentssSaasRefDocumentsGetQuery(
    {
      ...tenantIDHeader(),
     //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedSaasRefDocumentsData) {
      setIsDataReady(true);
      setRowCount(fetchedSaasRefDocumentsData.meta?.total_items);
    }
  }, [fetchedSaasRefDocumentsData, isLoadingData, isLoadingError]);
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/settings/configuration-management/documents/${row.original.id}`)}
                      >
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/settings/configuration-management/documents/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchSaasRefDocumentsList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateSaasRefDocuments({
        ...tenantIDHeader(),
        id: inpdata.id,
        saasRefDocumentsUpdate: inpdata
      }).unwrap();

      toast.success('Documents updated successfully');
      refetchSaasRefDocumentsList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <DocumentsGridForm
              ...
              refetchList={refetchSaasRefDocumentsList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this Documents ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedSaasRefDocumentsData ?? []}
            rowCount={rowCount}
