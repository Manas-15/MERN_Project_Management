...
import TenantCurrenciesForm from './TenantCurrenciesForm';
import TenantCurrenciesInlineForm from './TenantCurrenciesInlineForm';
import TenantCurrenciesGridForm from './TenantCurrenciesGridForm';
import TenantCurrenciesGlobalForm from './TenantCurrenciesGlobalForm';
import { 
...
const TenantCurrenciesListing : React.FC<{
  ...
  //FIXME
  const [updateSaasRefTenantCurrencies] = useUpdateSaasRefTenantCurrenciesSaasRefTenantCurrenciesIdPutMutation();
  const [deleteSaasRefTenantCurrencies] = useDeleteSaasRefTenantCurrenciesSaasRefTenantCurrenciesIdDeleteMutation();
  ...
  useEffect(() => {
   ...
   const {
    data: fetchedSaasRefTenantCurrenciesData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchSaasRefTenantCurrenciesList
  } = useGetAllSaasRefTenantCurrenciessSaasRefTenantCurrenciesGetQuery(
    {
      ...tenantIDHeader(),
     //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedSaasRefTenantCurrenciesData) {
      setIsDataReady(true);
      setRowCount(fetchedSaasRefTenantCurrenciesData.meta?.total_items);
    }
  }, [fetchedSaasRefTenantCurrenciesData, isLoadingData, isLoadingError]);
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/settings/configuration-management/tenant-currencies/${row.original.id}`)}
                      >
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/settings/configuration-management/tenant-currencies/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        await deleteSaasRefTenantCurrencies({
          ...tenantIDHeader(),
          id: selectedItem.id
        });
        setIsDeletePopupOpen(false);
        toast.success('Tenant Currencies deleted successfully');
        refetchSaasRefTenantCurrenciesList(); // Call refetch here
        setSelectedItem(null)
      } else {
        throw new Error('Selected Tenant Currencies ID is undefined');
      }
    } catch (error: any) {
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateSaasRefTenantCurrencies({
        ...tenantIDHeader(),
        id: inpdata.id,
        saasRefTenantCurrenciesUpdate: inpdata
      }).unwrap();

      toast.success('Tenant Currencies updated successfully');
      refetchSaasRefTenantCurrenciesList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        <Modal
          title={
            selectedItem ? 'Edit Tenant Currencies' : 'Create Tenant Currencies'
          }
          description=""
        ...
        >
            <TenantCurrenciesGridForm
              setOpen={setIsCreatePopupOpen}
              refetchList={refetchSaasRefTenantCurrenciesList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this TenantCurrencies ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedSaasRefTenantCurrenciesData ?? []}
            rowCount={rowCount}
...
export default TenantCurrenciesListing;