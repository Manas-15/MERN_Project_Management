...
import TenantConfigurationsInlineForm from './TenantConfigurationsInlineForm';
import TenantConfigurationsGridForm from './TenantConfigurationsGridForm';
import TenantConfigurationsGlobalForm from './TenantConfigurationsGlobalForm';
import {
...
const TenantConfigurationsListing: React.FC<{
  ...
  const {
    data: fetchedSetTenantConfigurationsData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchSetTenantConfigurationsList
  } = useGetAllSetTenantConfigurationssSetTenantConfigurationsGetQuery(
    {
      ...tenantIDHeader()
      //orderBy: orderBy,
  ...
  useEffect(() => {
    if (
      !isLoadingData &&
      !isLoadingError &&
      fetchedSetTenantConfigurationsData
    ) {
      setIsDataReady(true);
      setRowCount(fetchedSetTenantConfigurationsData.meta?.total_items);
    }
  }, [fetchedSetTenantConfigurationsData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
    () => [
          ...
          return (
            <div className='flex items-center'>
              <div
                ...
                onClick={() =>
                  router.push(
                    `/settings/configuration-management/tenant-configurations/${row.original.id}`
                  )
                }
              >
      ...
      {
        accessorKey: 'configurations_id',
        header: `Configurations Id`,
        size: 150,
        enableSorting: true,
        enableColumnFilter: false,
        enableEditing: true,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchSetTenantConfigurationsList(); // Call refetch here
  ...
  const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateSetTenantConfigurations({
        ...tenantIDHeader(),
        id: inpdata.id,
        setTenantConfigurationsUpdate: inpdata
      }).unwrap();

      toast.success('Tenant Configurations updated successfully');
      refetchSetTenantConfigurationsList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
          <TenantConfigurationsGridForm
            setOpen={setIsCreatePopupOpen}
            refetchList={refetchSetTenantConfigurationsList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this TenantConfigurations ?`}
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
          <MuiTanstackTable
            tableTitle=''
            columns={columns}
            importedData={fetchedSetTenantConfigurationsData ?? []}
            rowCount={rowCount}
...
export default TenantConfigurationsListing;
