...
import ModulesInlineForm from './ModulesInlineForm';
import ModulesGridForm from './ModulesGridForm';
import ModulesGlobalForm from './ModulesGlobalForm';
import { 
...
const ModulesListing : React.FC<{
  ...
  useEffect(() => {
   ...
   const {
    data: fetchedSaasRefModulesData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchSaasRefModulesList
  } = useGetAllSaasRefModulessSaasRefModulesGetQuery(
    {
      ...tenantIDHeader(),
     //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedSaasRefModulesData) {
      setIsDataReady(true);
      setRowCount(fetchedSaasRefModulesData.meta?.total_items);
    }
  }, [fetchedSaasRefModulesData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
      () => [
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/settings/configuration-management/modules/${row.original.id}`)}
                      >
          ...
          {
              accessorKey: 'module_display_name',
              header: `Module Display Name`,
              size: 150,
              enableSorting: true,
              enableColumnFilter: false,
              enableEditing: true,
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/settings/configuration-management/modules/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchSaasRefModulesList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateSaasRefModules({
        ...tenantIDHeader(),
        id: inpdata.id,
        saasRefModulesUpdate: inpdata
      }).unwrap();

      toast.success('Modules updated successfully');
      refetchSaasRefModulesList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <ModulesGridForm
              ...
              refetchList={refetchSaasRefModulesList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this Modules ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedSaasRefModulesData ?? []}
            rowCount={rowCount}
