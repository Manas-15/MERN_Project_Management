...
import SubscriptionFeaturesInlineForm from './SubscriptionFeaturesInlineForm';
import SubscriptionFeaturesGridForm from './SubscriptionFeaturesGridForm';
import SubscriptionFeaturesGlobalForm from './SubscriptionFeaturesGlobalForm';
import {
...
const SubscriptionFeaturesListing: React.FC<{
  ...
  const [updateSaasRefSubscriptionFeatures] =
    useUpdateSaasRefSubscriptionFeaturesSaasRefSubscriptionFeaturesIdPutMutation();
  const [deleteSaasRefSubscriptionFeatures] =
    useDeleteSaasRefSubscriptionFeaturesSaasRefSubscriptionFeaturesIdDeleteMutation();
  ...
  const {
    data: fetchedSaasRefSubscriptionFeaturesData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchSaasRefSubscriptionFeaturesList
  } = useGetAllSaasRefSubscriptionFeaturessSaasRefSubscriptionFeaturesGetQuery(
    {
      ...tenantIDHeader()
      //orderBy: orderBy,
  ...
  useEffect(() => {
    if (
      !isLoadingData &&
      !isLoadingError &&
      fetchedSaasRefSubscriptionFeaturesData
    ) {
      setIsDataReady(true);
      setRowCount(fetchedSaasRefSubscriptionFeaturesData.meta?.total_items);
    }
  }, [fetchedSaasRefSubscriptionFeaturesData, isLoadingData, isLoadingError]);
          ...
          return (
            <div className='flex items-center'>
              <div
                ...
                onClick={() =>
                  router.push(
                    `/settings/configuration-management/subscription-features/${row.original.id}`
                  )
                }
              >
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchSaasRefSubscriptionFeaturesList(); // Call refetch here
  ...
  const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateSaasRefSubscriptionFeatures({
        ...tenantIDHeader(),
        id: inpdata.id,
        saasRefSubscriptionFeaturesUpdate: inpdata
      }).unwrap();

      toast.success('Subscription Features updated successfully');
      refetchSaasRefSubscriptionFeaturesList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
          <SubscriptionFeaturesGridForm
            setOpen={setIsCreatePopupOpen}
            refetchList={refetchSaasRefSubscriptionFeaturesList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this SubscriptionFeatures ?`}
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
          <MuiTanstackTable
            tableTitle=''
            columns={columns}
            importedData={fetchedSaasRefSubscriptionFeaturesData ?? []}
            rowCount={rowCount}
...
export default SubscriptionFeaturesListing;
