...
import FeaturesInlineForm from './FeaturesInlineForm';
import FeaturesGridForm from './FeaturesGridForm';
import FeaturesGlobalForm from './FeaturesGlobalForm';
import { 
...
const FeaturesListing : React.FC<{
  ...
  //FIXME
  const [updateSaasRefFeatures] = useUpdateSaasRefFeaturesSaasRefFeaturesIdPutMutation();
  ...
  useEffect(() => {
   ...
   const {
    data: fetchedSaasRefFeaturesData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchSaasRefFeaturesList
  } = useGetAllSaasRefFeaturessSaasRefFeaturesGetQuery(
    {
      ...tenantIDHeader(),
     //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedSaasRefFeaturesData) {
      setIsDataReady(true);
      setRowCount(fetchedSaasRefFeaturesData.meta?.total_items);
    }
  }, [fetchedSaasRefFeaturesData, isLoadingData, isLoadingError]);
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/settings/configuration-management/features/${row.original.id}`)}
                      >
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/settings/configuration-management/features/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchSaasRefFeaturesList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateSaasRefFeatures({
        ...tenantIDHeader(),
        id: inpdata.id,
        saasRefFeaturesUpdate: inpdata
      }).unwrap();

      toast.success('Features updated successfully');
      refetchSaasRefFeaturesList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <FeaturesGridForm
              ...
              refetchList={refetchSaasRefFeaturesList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this Features ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedSaasRefFeaturesData ?? []}
            rowCount={rowCount}
