...
import TransactionDelaysInlineForm from './TransactionDelaysInlineForm';
import TransactionDelaysGridForm from './TransactionDelaysGridForm';
import TransactionDelaysGlobalForm from './TransactionDelaysGlobalForm';
import { 
...
const TransactionDelaysListing : React.FC<{
  ...
  const [deleteSetAstTransactionDelay] = useDeleteSetAstTransactionDelaySetAstTransactionDelayIdDeleteMutation();
  ...
  useEffect(() => {
   ...
   const {
    data: fetchedSetAstTransactionDelayData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchSetAstTransactionDelayList
  } = useGetAllSetAstTransactionDelaysSetAstTransactionDelayGetQuery(
    {
      ...tenantIDHeader(),
     //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedSetAstTransactionDelayData) {
      setIsDataReady(true);
      setRowCount(fetchedSetAstTransactionDelayData.meta?.total_items);
    }
  }, [fetchedSetAstTransactionDelayData, isLoadingData, isLoadingError]);
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/settings/miscellaneous/transaction-delays/${row.original.id}`)}
                      >
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/settings/miscellaneous/transaction-delays/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchSetAstTransactionDelayList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateSetAstTransactionDelay({
        ...tenantIDHeader(),
        id: inpdata.id,
        setAstTransactionDelayUpdate: inpdata
      }).unwrap();

      toast.success('Transaction Delays updated successfully');
      refetchSetAstTransactionDelayList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        <Modal
          title={
            selectedItem ? 'Edit Transaction Delays' : 'Create Transaction Delays'
          }
          description=""
        ...
        >
            <TransactionDelaysGridForm
              setOpen={setIsCreatePopupOpen}
              refetchList={refetchSetAstTransactionDelayList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this TransactionDelays ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedSetAstTransactionDelayData ?? []}
            rowCount={rowCount}
...
export default TransactionDelaysListing;