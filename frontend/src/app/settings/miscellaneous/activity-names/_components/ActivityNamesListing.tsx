...
import ActivityNamesInlineForm from './ActivityNamesInlineForm';
import ActivityNamesGridForm from './ActivityNamesGridForm';
import ActivityNamesGlobalForm from './ActivityNamesGlobalForm';
import { 
...
const ActivityNamesListing : React.FC<{
  ...
  //FIXME
  const [updateSetAstActivityNames] = useUpdateSetAstActivityNamesSetAstActivityNamesIdPutMutation();
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedSetAstActivityNamesData) {
      setIsDataReady(true);
      setRowCount(fetchedSetAstActivityNamesData.meta?.total_items);
    }
  }, [fetchedSetAstActivityNamesData, isLoadingData, isLoadingError]);
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/settings/miscellaneous/activity-names/${row.original.id}`)}
                      >
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/settings/miscellaneous/activity-names/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchSetAstActivityNamesList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateSetAstActivityNames({
        ...tenantIDHeader(),
        id: inpdata.id,
        setAstActivityNamesUpdate: inpdata
      }).unwrap();

      toast.success('Activity Names updated successfully');
      refetchSetAstActivityNamesList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <ActivityNamesGridForm
              setOpen={setIsCreatePopupOpen}
              refetchList={refetchSetAstActivityNamesList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this ActivityNames ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedSetAstActivityNamesData ?? []}
            rowCount={rowCount}
...
export default ActivityNamesListing;