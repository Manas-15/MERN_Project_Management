...
import WorkstationsInlineForm from './WorkstationsInlineForm';
import WorkstationsGridForm from './WorkstationsGridForm';
import WorkstationsGlobalForm from './WorkstationsGlobalForm';
import { 
...
const WorkstationsListing : React.FC<{
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedRefWorkstationData) {
      setIsDataReady(true);
      setRowCount(fetchedRefWorkstationData.meta?.total_items);
    }
  }, [fetchedRefWorkstationData, isLoadingData, isLoadingError]);
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/settings/miscellaneous/workstations/${row.original.id}`)}
                      >
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/settings/miscellaneous/workstations/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchRefWorkstationList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateRefWorkstation({
        ...tenantIDHeader(),
        id: inpdata.id,
        refWorkstationUpdate: inpdata
      }).unwrap();

      toast.success('Workstations updated successfully');
      refetchRefWorkstationList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <WorkstationsGridForm
              setOpen={setIsCreatePopupOpen}
              refetchList={refetchRefWorkstationList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this Workstations ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedRefWorkstationData ?? []}
            rowCount={rowCount}
...
export default WorkstationsListing;