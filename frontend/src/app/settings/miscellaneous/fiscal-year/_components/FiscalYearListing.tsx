...
import FiscalYearInlineForm from './FiscalYearInlineForm';
import FiscalYearGridForm from './FiscalYearGridForm';
import FiscalYearGlobalForm from './FiscalYearGlobalForm';
import { 
...
const FiscalYearListing : React.FC<{
  ...
  useEffect(() => {
   ...
   const {
    data: fetchedFiscalyearData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchFiscalyearList
  } = useGetAllFiscalyearsFiscalyearGetQuery(
    {
      ...tenantIDHeader(),
     //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedFiscalyearData) {
      setIsDataReady(true);
      setRowCount(fetchedFiscalyearData.meta?.total_items);
    }
  }, [fetchedFiscalyearData, isLoadingData, isLoadingError]);
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/settings/miscellaneous/fiscal-year/${row.original.id}`)}
                      >
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/settings/miscellaneous/fiscal-year/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchFiscalyearList(); // Call refetch here
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <FiscalYearGridForm
              ...
              refetchList={refetchFiscalyearList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this FiscalYear ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedFiscalyearData ?? []}
            rowCount={rowCount}
