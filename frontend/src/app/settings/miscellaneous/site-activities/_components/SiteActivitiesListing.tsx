...
import SiteActivitiesInlineForm from './SiteActivitiesInlineForm';
import SiteActivitiesGridForm from './SiteActivitiesGridForm';
import SiteActivitiesGlobalForm from './SiteActivitiesGlobalForm';
import { 
...
const SiteActivitiesListing : React.FC<{
  ...
  useEffect(() => {
   ...
   const {
    data: fetchedSetAstSiteActivityData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchSetAstSiteActivityList
  } = useGetAllSetAstSiteActivitysSetAstSiteActivityGetQuery(
    {
      ...tenantIDHeader(),
     //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedSetAstSiteActivityData) {
      setIsDataReady(true);
      setRowCount(fetchedSetAstSiteActivityData.meta?.total_items);
    }
  }, [fetchedSetAstSiteActivityData, isLoadingData, isLoadingError]);
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/settings/miscellaneous/site-activities/${row.original.id}`)}
                      >
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/settings/miscellaneous/site-activities/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchSetAstSiteActivityList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateSetAstSiteActivity({
        ...tenantIDHeader(),
        id: inpdata.id,
        setAstSiteActivityUpdate: inpdata
      }).unwrap();

      toast.success('Site Activities updated successfully');
      refetchSetAstSiteActivityList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <SiteActivitiesGridForm
              setOpen={setIsCreatePopupOpen}
              refetchList={refetchSetAstSiteActivityList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this SiteActivities ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedSetAstSiteActivityData ?? []}
            rowCount={rowCount}
...
export default SiteActivitiesListing;