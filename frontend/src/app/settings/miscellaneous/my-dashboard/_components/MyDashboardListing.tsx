...
import MyDashboardInlineForm from './MyDashboardInlineForm';
import MyDashboardGridForm from './MyDashboardGridForm';
import MyDashboardGlobalForm from './MyDashboardGlobalForm';
import { 
...
const MyDashboardListing : React.FC<{
  ...
  //FIXME
  const [updateSetGenMydashboard] = useUpdateSetGenMydashboardSetGenMydashboardIdPutMutation();
  ...
  useEffect(() => {
   ...
   const {
    data: fetchedSetGenMydashboardData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchSetGenMydashboardList
  } = useGetAllSetGenMydashboardsSetGenMydashboardGetQuery(
    {
      ...tenantIDHeader(),
     //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedSetGenMydashboardData) {
      setIsDataReady(true);
      setRowCount(fetchedSetGenMydashboardData.meta?.total_items);
    }
  }, [fetchedSetGenMydashboardData, isLoadingData, isLoadingError]);
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/settings/miscellaneous/my-dashboard/${row.original.id}`)}
                      >
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/settings/miscellaneous/my-dashboard/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchSetGenMydashboardList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateSetGenMydashboard({
        ...tenantIDHeader(),
        id: inpdata.id,
        setGenMydashboardUpdate: inpdata
      }).unwrap();

      toast.success('My Dashboard updated successfully');
      refetchSetGenMydashboardList();
    } catch (error) {
      toast.error('Failed to update My Dashboard');
    }
  };
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <MyDashboardGridForm
              ...
              refetchList={refetchSetGenMydashboardList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this MyDashboard ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedSetGenMydashboardData ?? []}
            rowCount={rowCount}
...
export default MyDashboardListing;