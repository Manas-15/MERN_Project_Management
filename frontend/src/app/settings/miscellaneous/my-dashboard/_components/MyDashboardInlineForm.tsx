...
export default function MyDashboardInlineForm({
  setOpen,
  refetchList,
  selectedItem,
  setSelectedItem,
...
const [createSetGenMydashboard] = useCreateSetGenMydashboardSetGenMydashboardPostMutation();
    ...
    if (selectedItem) {
      try {
        await updateSetGenMydashboard({
           ...tenantIDHeader(),
          id: selectedItem.id,
          setGenMydashboardUpdate: values
        }).unwrap();
      ...
      try {
        await createSetGenMydashboard({
           ...tenantIDHeader(),
          setGenMydashboardInsert: values
        }).unwrap();
        toast.success(`My Dashboard created successfully`);
        afterSubmit();
      } catch (error) {
        toast.error('Failed to create My Dashboard');
      }
    }
  }
             ...
             <CardContent className='form-dynamic-height overflow-y-auto px-6'>
                              ...
                              <FormField
                                ...
                                render={({ field }) => (
                                  <FormItem viewType='inline' className='flex items-center'>
                                    <FormLabel className='w-[15%]'>Widget</FormLabel>
                          ...
                          <div className="mb-6 grid grid-cols-1 gap-6">
                              <FormField
                                ...
                                render={({ field }) => (
                                 <FormItem viewType='inline' className='flex items-center'>
                                    <FormLabel className='w-[15%]'>Sequence</FormLabel>
                                    <div className='flex w-[85%] flex-col'>
                                      <FormControl>
