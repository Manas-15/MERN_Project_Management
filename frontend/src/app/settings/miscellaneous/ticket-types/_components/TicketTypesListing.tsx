...
import TicketTypesInlineForm from './TicketTypesInlineForm';
import TicketTypesGridForm from './TicketTypesGridForm';
import TicketTypesGlobalForm from './TicketTypesGlobalForm';
import { 
...
const TicketTypesListing : React.FC<{
  ...
  //FIXME
  const [updateSetAstTicketType] = useUpdateSetAstTicketTypeSetAstTicketTypeIdPutMutation();
  ...
  useEffect(() => {
   ...
   const {
    data: fetchedSetAstTicketTypeData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchSetAstTicketTypeList
  } = useGetAllSetAstTicketTypesSetAstTicketTypeGetQuery(
    {
      ...tenantIDHeader(),
     //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedSetAstTicketTypeData) {
      setIsDataReady(true);
      setRowCount(fetchedSetAstTicketTypeData.meta?.total_items);
    }
  }, [fetchedSetAstTicketTypeData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
      () => [
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/settings/miscellaneous/ticket-types/${row.original.id}`)}
                      >
          ...
          {
              accessorKey: 'ticket_type_note',
              header: `Ticket Type Note`,
              size: 150,
              enableSorting: true,
              enableColumnFilter: false,
              enableEditing: true,
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/settings/miscellaneous/ticket-types/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchSetAstTicketTypeList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateSetAstTicketType({
        ...tenantIDHeader(),
        id: inpdata.id,
        setAstTicketTypeUpdate: inpdata
      }).unwrap();

      toast.success('Ticket Types updated successfully');
      refetchSetAstTicketTypeList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <TicketTypesGridForm
              ...
              refetchList={refetchSetAstTicketTypeList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this TicketTypes ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedSetAstTicketTypeData ?? []}
            rowCount={rowCount}
...
export default TicketTypesListing;