...
import VendorsInlineForm from './VendorsInlineForm';
import VendorsGridForm from './VendorsGridForm';
import VendorsGlobalForm from './VendorsGlobalForm';
import { 
...
const VendorsListing : React.FC<{
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedRefVendorData) {
      setIsDataReady(true);
      setRowCount(fetchedRefVendorData.meta?.total_items);
    }
  }, [fetchedRefVendorData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
      () => [
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/settings/vendors-parties/vendors/${row.original.id}`)}
                      >
          ...
          {
              accessorKey: 'authorization_name',
          ...
          {
              accessorKey: 'area_of_ops_state',
              header: `Area Of Ops State`,
              size: 150,
              enableSorting: true,
              enableColumnFilter: false,
              enableEditing: true,
          ...
          {
              accessorKey: 'area_of_ops_district',
              header: `Area Of Ops District`,
              size: 150,
              enableSorting: true,
              enableColumnFilter: false,
              enableEditing: true,
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/settings/vendors-parties/vendors/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchRefVendorList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateRefVendor({
        ...tenantIDHeader(),
        id: inpdata.id,
        refVendorUpdate: inpdata
      }).unwrap();

      toast.success('Vendors updated successfully');
      refetchRefVendorList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        <Modal
          title={
            selectedItem ? 'Edit Vendors' : 'Create Vendors'
          }
          description=""
        ...
        >
            <VendorsGridForm
              ...
              refetchList={refetchRefVendorList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this Vendors ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedRefVendorData ?? []}
            rowCount={rowCount}
