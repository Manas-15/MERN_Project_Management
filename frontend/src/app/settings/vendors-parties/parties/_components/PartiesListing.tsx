...
import PartiesInlineForm from './PartiesInlineForm';
import PartiesGridForm from './PartiesGridForm';
import PartiesGlobalForm from './PartiesGlobalForm';
import { 
...
const PartiesListing : React.FC<{
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedRefPartyData) {
      setIsDataReady(true);
      setRowCount(fetchedRefPartyData.meta?.total_items);
    }
  }, [fetchedRefPartyData, isLoadingData, isLoadingError]);
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/settings/vendors-parties/parties/${row.original.id}`)}
                      >
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/settings/vendors-parties/parties/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchRefPartyList(); // Call refetch here
  ...
  return (
    <>
      {isCreatePopupOpen && (
        <Modal
          title={
            selectedItem ? 'Edit Parties' : 'Create Parties'
          }
          description=""
        ...
        >
            <PartiesGridForm
              ...
              refetchList={refetchRefPartyList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this Parties ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedRefPartyData ?? []}
            rowCount={rowCount}
