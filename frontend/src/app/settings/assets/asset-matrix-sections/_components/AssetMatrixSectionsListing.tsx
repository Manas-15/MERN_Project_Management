...
import AssetMatrixSectionsInlineForm from './AssetMatrixSectionsInlineForm';
import AssetMatrixSectionsGridForm from './AssetMatrixSectionsGridForm';
import AssetMatrixSectionsGlobalForm from './AssetMatrixSectionsGlobalForm';
import { 
...
const AssetMatrixSectionsListing : React.FC<{
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedAssetMatrixSectionData) {
      setIsDataReady(true);
      setRowCount(fetchedAssetMatrixSectionData.meta?.total_items);
    }
  }, [fetchedAssetMatrixSectionData, isLoadingData, isLoadingError]);
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/settings/assets/asset-matrix-sections/${row.original.id}`)}
                      >
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/settings/assets/asset-matrix-sections/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchAssetMatrixSectionList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateAssetMatrixSection({
        ...tenantIDHeader(),
        id: inpdata.id,
        assetMatrixSectionUpdate: inpdata
      }).unwrap();

      toast.success('Asset Matrix Sections updated successfully');
      refetchAssetMatrixSectionList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <AssetMatrixSectionsGridForm
              setOpen={setIsCreatePopupOpen}
              refetchList={refetchAssetMatrixSectionList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this AssetMatrixSections ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedAssetMatrixSectionData ?? []}
            rowCount={rowCount}
...
export default AssetMatrixSectionsListing;