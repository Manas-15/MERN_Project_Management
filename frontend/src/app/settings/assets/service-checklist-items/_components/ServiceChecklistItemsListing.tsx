...
import ServiceChecklistItemsInlineForm from './ServiceChecklistItemsInlineForm';
import ServiceChecklistItemsGridForm from './ServiceChecklistItemsGridForm';
import ServiceChecklistItemsGlobalForm from './ServiceChecklistItemsGlobalForm';
import { 
...
const ServiceChecklistItemsListing : React.FC<{
  ...
  useEffect(() => {
   ...
   const {
    data: fetchedAssetServiceCheckListItemsData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchAssetServiceCheckListItemsList
  } = useGetAllAssetServiceCheckListItemssAssetServiceCheckListItemsGetQuery(
    {
      ...tenantIDHeader(),
     //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedAssetServiceCheckListItemsData) {
      setIsDataReady(true);
      setRowCount(fetchedAssetServiceCheckListItemsData.meta?.total_items);
    }
  }, [fetchedAssetServiceCheckListItemsData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
      () => [
          {
              accessorKey: 'assetmatrix_id',
              header: `Assetmatrix Id`,
              size: 150,
              enableSorting: true,
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/settings/assets/service-checklist-items/${row.original.id}`)}
                      >
          ...
          {
              accessorKey: 'service_check_list_id',
              header: `Service Check List Id`,
              size: 150,
              enableSorting: true,
              enableColumnFilter: false,
              enableEditing: true,
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/settings/assets/service-checklist-items/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchAssetServiceCheckListItemsList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateAssetServiceCheckListItems({
        ...tenantIDHeader(),
        id: inpdata.id,
        assetServiceCheckListItemsUpdate: inpdata
      }).unwrap();

      toast.success('Service Checklist Items updated successfully');
      refetchAssetServiceCheckListItemsList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <ServiceChecklistItemsGridForm
              setOpen={setIsCreatePopupOpen}
              refetchList={refetchAssetServiceCheckListItemsList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this ServiceChecklistItems ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedAssetServiceCheckListItemsData ?? []}
            rowCount={rowCount}
...
export default ServiceChecklistItemsListing;