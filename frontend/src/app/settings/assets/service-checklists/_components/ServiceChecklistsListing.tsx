...
import ServiceChecklistsInlineForm from './ServiceChecklistsInlineForm';
import ServiceChecklistsGridForm from './ServiceChecklistsGridForm';
import ServiceChecklistsGlobalForm from './ServiceChecklistsGlobalForm';
import { 
...
const ServiceChecklistsListing : React.FC<{
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedAssetServiceCheckListData) {
      setIsDataReady(true);
      setRowCount(fetchedAssetServiceCheckListData.meta?.total_items);
    }
  }, [fetchedAssetServiceCheckListData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
      () => [
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/settings/assets/service-checklists/${row.original.id}`)}
                      >
          ...
          {
              accessorKey: 'hour_meter_flag',
              header: `Hour Meter Flag`,
              size: 150,
              enableSorting: true,
              enableColumnFilter: false,
              enableEditing: true,
          ...
          {
              accessorKey: 'drum_hour_flag',
              header: `Drum Hour Flag`,
              size: 150,
              enableSorting: true,
              enableColumnFilter: false,
              enableEditing: true,
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/settings/assets/service-checklists/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchAssetServiceCheckListList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateAssetServiceCheckList({
        ...tenantIDHeader(),
        id: inpdata.id,
        assetServiceCheckListUpdate: inpdata
      }).unwrap();

      toast.success('Service Checklists updated successfully');
      refetchAssetServiceCheckListList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <ServiceChecklistsGridForm
              setOpen={setIsCreatePopupOpen}
              refetchList={refetchAssetServiceCheckListList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this ServiceChecklists ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedAssetServiceCheckListData ?? []}
            rowCount={rowCount}
...
export default ServiceChecklistsListing;