...
import AssetTypeGroupsInlineForm from './AssetTypeGroupsInlineForm';
import AssetTypeGroupsGridForm from './AssetTypeGroupsGridForm';
import AssetTypeGroupsGlobalForm from './AssetTypeGroupsGlobalForm';
import { 
...
const AssetTypeGroupsListing : React.FC<{
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedSetAstAssetTypeGroupData) {
      setIsDataReady(true);
      setRowCount(fetchedSetAstAssetTypeGroupData.meta?.total_items);
    }
  }, [fetchedSetAstAssetTypeGroupData, isLoadingData, isLoadingError]);
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/settings/assets/asset-type-groups/${row.original.id}`)}
                      >
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/settings/assets/asset-type-groups/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchSetAstAssetTypeGroupList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateSetAstAssetTypeGroup({
        ...tenantIDHeader(),
        id: inpdata.id,
        setAstAssetTypeGroupUpdate: inpdata
      }).unwrap();

      toast.success('Asset Type Groups updated successfully');
      refetchSetAstAssetTypeGroupList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <AssetTypeGroupsGridForm
              setOpen={setIsCreatePopupOpen}
              refetchList={refetchSetAstAssetTypeGroupList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this AssetTypeGroups ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedSetAstAssetTypeGroupData ?? []}
            rowCount={rowCount}
...
export default AssetTypeGroupsListing;