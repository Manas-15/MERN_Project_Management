...
const SubAssembliesListing : React.FC<{
  ...
  const [deleteRefSubAssembly] = useDeleteRefSubAssemblyRefSubAssemblyIdDeleteMutation();
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/settings/assets/sub-assemblies/${row.original.id}`)}
                      >
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/settings/assets/sub-assemblies/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  return (
    <>
      {isCreatePopupOpen && (
        <Modal
          title={
            selectedItem ? 'Edit Sub Assemblies' : 'Create Sub Assemblies'
          }
          description=""
