...
import AssetMatrixInlineForm from './AssetMatrixInlineForm';
import AssetMatrixGridForm from './AssetMatrixGridForm';
import AssetMatrixGlobalForm from './AssetMatrixGlobalForm';
import { 
...
const AssetMatrixListing : React.FC<{
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedAssetMatrixData) {
      setIsDataReady(true);
      setRowCount(fetchedAssetMatrixData.meta?.total_items);
    }
  }, [fetchedAssetMatrixData, isLoadingData, isLoadingError]);
  ...
  const columns = useMemo<any>(
      () => [
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/settings/assets/asset-matrix/${row.original.id}`)}
                      >
          ...
          {
              accessorKey: 'asset_capacity',
              header: `Asset Capacity`,
              size: 150,
              enableSorting: true,
              enableColumnFilter: false,
              enableEditing: true,
          ...
          {
              accessorKey: 'fuel_tank_capacity',
          ...
          {
              accessorKey: 'electrical_systme',
              header: `Electrical Systme`,
              size: 150,
              enableSorting: true,
              enableColumnFilter: false,
              enableEditing: true,
          ...
          {
              accessorKey: 'operation_cycle',
              header: `Operation Cycle`,
              size: 150,
              enableSorting: true,
              enableColumnFilter: false,
              enableEditing: true,
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/settings/assets/asset-matrix/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchAssetMatrixList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateAssetMatrix({
        ...tenantIDHeader(),
        id: inpdata.id,
        assetMatrixUpdate: inpdata
      }).unwrap();

      toast.success('Asset Matrix updated successfully');
      refetchAssetMatrixList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <AssetMatrixGridForm
              ...
              refetchList={refetchAssetMatrixList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this AssetMatrix ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedAssetMatrixData ?? []}
            rowCount={rowCount}
...
export default AssetMatrixListing;