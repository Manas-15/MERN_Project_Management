...
export default function OwnerCategoriesInlineForm({
  setOpen,
  refetchList,
  selectedItem,
  setSelectedItem,
...
const [createSetAstOwnerCategory] = useCreateSetAstOwnerCategorySetAstOwnerCategoryPostMutation();
...
const defaultValues = {
         owner_type: selectedItem?.owner_type || "",
    ...
    if (selectedItem) {
      try {
        await updateSetAstOwnerCategory({
           ...tenantIDHeader(),
          id: selectedItem.id,
          setAstOwnerCategoryUpdate: values
        }).unwrap();
      ...
      try {
        await createSetAstOwnerCategory({
           ...tenantIDHeader(),
          setAstOwnerCategoryInsert: values
        }).unwrap();
        toast.success(`Owner Categories created successfully`);
        afterSubmit();
      } catch (error) {
        toast.error('Failed to create Owner Categories');
      }
    }
  }
             ...
             <CardContent className='form-dynamic-height overflow-y-auto px-6'>
                              ...
                              <FormField
                                ...
                                render={({ field }) => (
                                  <FormItem viewType='inline' className='flex items-center'>
                                    <FormLabel className='w-[15%]'>Owner Type</FormLabel>
                          ...
                          <div className="mb-6 grid grid-cols-1 gap-6">
                              <FormField
                                ...
                                render={({ field }) => (
                                  <FormItem viewType='inline' className='flex items-center'>
                                    <FormLabel className='w-[15%]'>Owner Name</FormLabel>
                                    <div className='flex w-[85%] flex-col'>
                                        <FormControl>
                                          <Input type='text' {...field} />
                                        </FormControl>
