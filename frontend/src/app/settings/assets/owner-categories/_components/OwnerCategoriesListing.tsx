...
import OwnerCategoriesInlineForm from './OwnerCategoriesInlineForm';
import OwnerCategoriesGridForm from './OwnerCategoriesGridForm';
import OwnerCategoriesGlobalForm from './OwnerCategoriesGlobalForm';
import { 
...
const OwnerCategoriesListing : React.FC<{
  ...
  //FIXME
  const [updateSetAstOwnerCategory] = useUpdateSetAstOwnerCategorySetAstOwnerCategoryIdPutMutation();
  ...
  useEffect(() => {
   ...
   const {
    data: fetchedSetAstOwnerCategoryData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchSetAstOwnerCategoryList
  } = useGetAllSetAstOwnerCategorysSetAstOwnerCategoryGetQuery(
    {
      ...tenantIDHeader(),
     //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedSetAstOwnerCategoryData) {
      setIsDataReady(true);
      setRowCount(fetchedSetAstOwnerCategoryData.meta?.total_items);
    }
  }, [fetchedSetAstOwnerCategoryData, isLoadingData, isLoadingError]);
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/settings/assets/owner-categories/${row.original.id}`)}
                      >
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/settings/assets/owner-categories/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchSetAstOwnerCategoryList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateSetAstOwnerCategory({
        ...tenantIDHeader(),
        id: inpdata.id,
        setAstOwnerCategoryUpdate: inpdata
      }).unwrap();

      toast.success('Owner Categories updated successfully');
      refetchSetAstOwnerCategoryList();
    } catch (error) {
      toast.error('Failed to update Owner Categories');
    }
  };
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <OwnerCategoriesGridForm
              setOpen={setIsCreatePopupOpen}
              refetchList={refetchSetAstOwnerCategoryList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this OwnerCategories ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedSetAstOwnerCategoryData ?? []}
            rowCount={rowCount}
...
export default OwnerCategoriesListing;