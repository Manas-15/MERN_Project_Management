...
import MaterialsInlineForm from './MaterialsInlineForm';
import MaterialsGridForm from './MaterialsGridForm';
import MaterialsGlobalForm from './MaterialsGlobalForm';
import { 
...
const MaterialsListing : React.FC<{
  ...
  useEffect(() => {
   ...
   const {
    data: fetchedRefAstMaterialData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchRefAstMaterialList
  } = useGetAllRefAstMaterialsRefAstMaterialGetQuery(
    {
      ...tenantIDHeader(),
     //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedRefAstMaterialData) {
      setIsDataReady(true);
      setRowCount(fetchedRefAstMaterialData.meta?.total_items);
    }
  }, [fetchedRefAstMaterialData, isLoadingData, isLoadingError]);
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/settings/assets/materials/${row.original.id}`)}
                      >
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/settings/assets/materials/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchRefAstMaterialList(); // Call refetch here
   ...
   const inlineEditApi = async (inpdata: any) => {
    ...
    try {
      ...
      await updateRefAstMaterial({
        ...tenantIDHeader(),
        id: inpdata.id,
        refAstMaterialUpdate: inpdata
      }).unwrap();

      toast.success('Materials updated successfully');
      refetchRefAstMaterialList();
    } catch (error) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <MaterialsGridForm
              ...
              refetchList={refetchRefAstMaterialList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this Materials ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedRefAstMaterialData ?? []}
            rowCount={rowCount}
