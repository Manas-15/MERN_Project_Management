...
import UnitsOfMeasureInlineForm from './UnitsOfMeasureInlineForm';
import UnitsOfMeasureGridForm from './UnitsOfMeasureGridForm';
import UnitsOfMeasureGlobalForm from './UnitsOfMeasureGlobalForm';
import { 
...
const UnitsOfMeasureListing : React.FC<{
  ...
  useEffect(() => {
   ...
   const {
    data: fetchedSetInvUomData,
    isError: isLoadingError,
    isFetching: isFetchingData,
    isLoading: isLoadingData,
    refetch: refetchSetInvUomList
  } = useGetAllSetInvUomsSetInvUomGetQuery(
    {
      ...tenantIDHeader(),
     //orderBy: orderBy,
  ...
  useEffect(() => {
    if (!isLoadingData && !isLoadingError && fetchedSetInvUomData) {
      setIsDataReady(true);
      setRowCount(fetchedSetInvUomData.meta?.total_items);
    }
  }, [fetchedSetInvUomData, isLoadingData, isLoadingError]);
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/settings/inventory/units-of-measure/${row.original.id}`)}
                      >
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/settings/inventory/units-of-measure/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        refetchSetInvUomList(); // Call refetch here
  ...
  return (
    <>
      {isCreatePopupOpen && (
        ...
        >
            <UnitsOfMeasureGridForm
              setOpen={setIsCreatePopupOpen}
              refetchList={refetchSetInvUomList}
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this UnitsOfMeasure ?`} 
      ...
      <ErrorBoundary fallback={<div>Error loading data!</div>}>
        {isDataReady ? (
         <MuiTanstackTable
            tableTitle=""
            columns={columns}
            importedData={fetchedSetInvUomData ?? []}
            rowCount={rowCount}
...
export default UnitsOfMeasureListing;