...
import TaxesInlineForm from './TaxesInlineForm';
import TaxesGridForm from './TaxesGridForm';
import TaxesGlobalForm from './TaxesGlobalForm';
import { 
              ...
              Cell: ({ cell, row }: any) => {
                ...
                return (
                  <div className="flex items-center"><div
                        ...
                        onClick={() => router.push(`/settings/inventory/taxes/${row.original.id}`)}
                      >
  ...
  const kababMenuOptions = [
    {
      ...
      size: 20,
      onClick: (row: any) =>
       router.push(`/settings/inventory/taxes/${row.original.id}`)
    },
    {
      label: 'Edit',
      icon: Pencil,
  ...
  const onConfirm = async () => {
    ...
    try {
      if (selectedItem?.id) {
        ...
        toast.success('Taxes deleted successfully');
        refetchRefTaxList(); // Call refetch here
        setSelectedItem(null)
      } else {
        throw new Error('Selected Taxes ID is undefined');
      }
    } catch (error: any) {
  ...
  return (
    <>
      {isCreatePopupOpen && (
        <Modal
          title={
            selectedItem ? 'Edit Taxes' : 'Create Taxes'
          }
          description=""
      ...
      <AlertModal
        ...
        onConfirm={onConfirm}
        loading={false}
        message={`Are you sure you want to remove this Taxes ?`} 
