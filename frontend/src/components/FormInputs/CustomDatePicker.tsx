  ...
  return (
    <FormField
      ...
      render={({ field }) => (
        <FormItem
          viewType={viewType}
          className={`${viewType == 'inline' ? 'flex items-center' : ''}`}
        >
          <FormLabel className={`${viewType == 'inline' ? 'w-[15%]' : ''}`}>
            {label}
            {isRequired && <span className='mx-2 text-red-500'>*</span>}
          </FormLabel>
          <div
            className={`${viewType == 'inline' ? 'flex w-[85%] flex-col' : ''}`}
          >
            <FormControl>
              <div className='px-4.5 dt_picker border-stroke relative mr-2 rounded border bg-white py-2'>
                <DatePicker
                  ref={DatePickerRef}
                  className='custom-date-picker font-normal'
                  autoComplete='off'
